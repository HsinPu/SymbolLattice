...
import { renderExploreText } from "./explore-text.js";
...
/** Converts the internal typed explore response into the text-only MCP surface. */
export function renderExploreToolTextResponse(response: ExploreToolResponse): ExploreToolResponse {
  if (response.isError === true || response.structuredContent === undefined) return response;
  return {
    content: [{ type: "text", text: renderExploreText(response.structuredContent) }]
  };
}
...
export function createMcpServer(
  ...
  server.registerTool(
    "SymbolLattice_explore",
    {
      ...
      description:
        "Primary read-equivalent code-intelligence tool. Call it before Read or Grep for code questions, architecture, flows, bug fixes, feature work, or a named file or symbol. It returns concise Markdown with generation-bound line-numbered source, exact connections and paths, plus bounded impact context; treat returned source as already read. It reports index freshness and never creates or refreshes an index.",
      inputSchema: {
