...
const LANGUAGE_LIMITATIONS: Readonly<Partial<Record<ArtifactLanguage, readonly string[]>>> =
  Object.freeze({
    typescript: ["parse-rejected, overload, conditional export, and runtime dispatch remain nonclaims", "Named function expressions retain lexical self references. Anonymous functions/arrows assigned to static member paths retain the full assignment source and internal call ownership; labels are not lexical bindings, export proof or receiver-dispatch proof. Dynamic computed targets and unassigned anonymous callbacks are outside this addition"],
    javascript: ["300-positive/150-negative ESTree truth covers bounded same-file relations and unique relative ESM or strict CommonJS imports; dynamic imports, package resolution, re-exports, mutable globals, and runtime dispatch remain nonclaims", "Named function expressions retain lexical self references. Anonymous functions/arrows assigned to static member paths retain the full assignment source and internal call ownership; labels are not lexical bindings, export proof or receiver-dispatch proof. Dynamic computed targets and unassigned anonymous callbacks are outside this addition", "CommonJS object-call resolution requires strict JavaScript or .cjs, const named destructuring from a relative .js/.cjs file, and unique unmutated top-level function exports. Stable properties of a top-level const object exported as module.exports can additionally link destructured new-site names back to their property declaration; this is a source reference, not proof of the runtime constructor. Dynamic exports, namespace calls, default exports, directory/package lookup, ESM interop and cyclic imports remain unsupported; observed module-object mutation or escape suppresses affected targets"],
    python: ["340-positive/150-negative CPython AST truth covers prior imports, direct calls, construction and inheritance plus parser-clean unique direct self member calls; decorators, metaclasses, attribute hooks, mutation, dynamic loading, monkey patching, inherited dispatch, and parser-rejected files remain nonclaims", "Direct same-class self calls now include async methods as callers and targets. A pinned Django CPython AST audit checked 4,097 emitted source/target receipts and five selected async calls; it does not establish runtime dispatch or corpus-wide recall. General bare-name calls and construction inside async methods retain their conservative boundary", "Direct module assignments to one ASCII name, with an optional type annotation and a value, produce variable declarations and containment evidence. They do not establish runtime values, exports or alias/call targets. Conditional, chained, destructuring, attribute, annotation-only and parser-rejected assignments remain outside this addition", "In parser-clean function and method bodies, static dotted member invocations without existing exact call evidence retain unresolved edges with full callee ranges, null targets and zero confidence. Async, decorated and nested function bodies retain lexical ownership. Lambda bodies, module/class execution, decorators, default arguments, computed receivers and parser-rejected modules are excluded; eligible closed CRLF recovery follows the existing parser contract. These receipts do not prove receiver types, targets or dispatch. See benchmarks/python/member-calls.mjs for independent CPython source-range validation"],
...
function truthKind(language: ArtifactLanguage): LanguageTruthKind {
  ...
  if (language === "cobol") return "gnucobol-xref-listing";
