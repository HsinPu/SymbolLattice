...
export function validateDisposableRProject(projectPath, probePath) {
  const project = resolve(projectPath);
  const probe = resolve(project, probePath);
  const relativeProbe = relative(project, probe);
  if (!basename(project).startsWith("r-v0437-") || !existsSync(join(project, ".SymbolLattice")) || isAbsolute(relativeProbe) || relativeProbe.startsWith("..") || !probe.toLowerCase().endsWith(".r")) {
    throw new Error("Lifecycle mutations require an indexed r-v0437-* disposable project and an in-project R probe.");
  }
...
export function runRLifecycle({ projectPath, probePath }) {
  const { project, probe } = validateDisposableRProject(projectPath, probePath);
  const original = readFileSync(probe);
  const originalHash = sha256(original);
  const renamed = join(dirname(probe), `${basename(probe, ".r")}.renamed.r`);
  const deleted = join(dirname(project), `${basename(project)}-deleted-probe.r`);
  ...
  try {
    ...
    writeFileSync(probe, Buffer.concat([original, Buffer.from("\n\nadded <- function(value) { value }\nsetClass(\"AddedRecord\", slots = list(value = \"character\"))\n", "utf8")]));
    const semantic = runCli("sync", project);
    if (semantic.counts.symbols !== baseline.counts.symbols + 2 || semantic.counts.edges !== baseline.counts.edges + 2) throw new Error("Semantic mutation did not add exactly one R function and one S4 class with containment.");
    ...
    if (removal.counts.files !== baseline.counts.files - 1) throw new Error("Delete did not remove exactly one indexed R file.");
    ...
    if (!sameCounts(restore.counts, baseline.counts) || reopen.generationId !== restore.generationId || !sameCounts(reopen.counts, baseline.counts)) throw new Error("Restore or fresh-process reopen did not retain baseline R graph.");
    ...
    return { schemaVersion: 1, benchmark: "symbollattice-r-large-project-lifecycle-v1", project, probePath, originalSha256: originalHash, evidence, acceptance: { status: "pass" } };
  } finally {
...
if (process.argv[1] !== undefined && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if ([projectPath, probePath, output].some((value) => value === null)) throw new Error("Usage: --project <disposable project> --probe <relative.r> --output <json>");
  const report = runRLifecycle({ projectPath, probePath });
