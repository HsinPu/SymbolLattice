...
export function validateDisposableJuliaProject(projectPath, probePath) {
  const project = resolve(projectPath);
  const probe = resolve(project, probePath);
  const relativeProbe = relative(project, probe);
  if (!basename(project).startsWith("julia-v0435-") || !existsSync(join(project, ".SymbolLattice")) || isAbsolute(relativeProbe) || relativeProbe.startsWith("..") || !probe.toLowerCase().endsWith(".jl")) {
    throw new Error("Lifecycle mutations require an indexed julia-v0435-* disposable project and an in-project Julia probe.");
  }
...
function runCli(command, project) {
  ...
  const result = spawnSync(process.execPath, [cli, command, project, "--json"], { cwd: dirname(cli), encoding: "utf8", maxBuffer: 32 * 1024 * 1024 });
  if (result.status !== 0) throw new Error(`${command} failed: ${result.stderr || result.stdout}`);
  return JSON.parse(result.stdout);
}

function sameCounts(left, right) { return ["files", "symbols", "edges", "pendingReferences"].every((key) => left[key] === right[key]); }

export function runJuliaLifecycle({ projectPath, probePath }) {
  const { project, probe } = validateDisposableJuliaProject(projectPath, probePath);
  const original = readFileSync(probe);
  const originalHash = sha256(original);
  const renamed = join(dirname(probe), `${basename(probe, ".jl")}.renamed.jl`);
  const deleted = join(dirname(project), `${basename(project)}-deleted-probe.jl`);
  ...
  try {
    ...
    writeFileSync(probe, Buffer.concat([original, Buffer.from("\n\nstruct LifecycleRecord\n    value::String\nend\n\nfunction lifecycleAdded(record::LifecycleRecord)\n    record.value\nend\n", "utf8")]));
    const semantic = runCli("sync", project);
    if (semantic.counts.symbols !== baseline.counts.symbols + 2 || semantic.counts.edges !== baseline.counts.edges + 2) throw new Error("Semantic mutation did not add exactly one Julia type and one function with containment.");
    ...
    if (removal.counts.files !== baseline.counts.files - 1) throw new Error("Delete did not remove exactly one indexed Julia file.");
    ...
    if (!sameCounts(restore.counts, baseline.counts) || reopen.generationId !== restore.generationId || !sameCounts(reopen.counts, baseline.counts)) throw new Error("Restore or fresh-process reopen did not retain baseline Julia graph.");
    ...
    return { schemaVersion: 1, benchmark: "symbollattice-julia-large-project-lifecycle-v1", project, probePath, originalSha256: originalHash, evidence, acceptance: { status: "pass" } };
  } finally {
...
if (process.argv[1] !== undefined && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if ([projectPath, probePath, output].some((value) => value === null)) throw new Error("Usage: --project <disposable project> --probe <relative.jl> --output <json>");
  const report = runJuliaLifecycle({ projectPath, probePath });
