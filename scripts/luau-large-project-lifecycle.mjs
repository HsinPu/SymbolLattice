...
export function validateDisposableLuauProject(projectPath, probePath) {
  const project = resolve(projectPath);
  const probe = resolve(project, probePath);
  const relativeProbe = relative(project, probe);
  if (
    !basename(project).startsWith("luau-v0434-") ||
    !existsSync(join(project, ".SymbolLattice")) ||
    isAbsolute(relativeProbe) ||
    relativeProbe.startsWith("..") ||
    !probe.toLowerCase().endsWith(".luau")
  ) {
    throw new Error("Lifecycle mutations require an indexed luau-v0434-* disposable project and an in-project Luau probe.");
  }
...
export function runLifecycle({ projectPath, probePath }) {
  const { project, probe } = validateDisposableLuauProject(projectPath, probePath);
  const original = readFileSync(probe);
  const originalHash = sha256(original);
  const renamed = join(dirname(probe), `${basename(probe, ".luau")}.renamed.luau`);
  const deleted = join(dirname(project), `${basename(project)}-deleted-probe.luau`);
  ...
  try {
    ...
    writeFileSync(probe, Buffer.concat([original, Buffer.from(`\n\nlocal function lifecycleAdded<T>(value: T): T\n  return value\nend\n\nexport type LifecycleRecord = { value: string }\n`, "utf8")]));
    const semantic = runCli("sync", project);
    if (semantic.counts.symbols !== baseline.counts.symbols + 2 || semantic.counts.edges !== baseline.counts.edges + 2) throw new Error("Semantic mutation did not add exactly one function and one type with containment.");
    ...
    if (removal.counts.files !== baseline.counts.files - 1) throw new Error("Delete did not remove exactly one indexed Luau file.");
    ...
    return { schemaVersion: 1, benchmark: "symbollattice-luau-large-project-lifecycle-v1", project, probePath, originalSha256: originalHash, evidence, acceptance: { status: "pass" } };
  } finally {
...
if (process.argv[1] !== undefined && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if ([projectPath, probePath, output].some((value) => value === null)) throw new Error("Usage: --project <disposable project> --probe <relative.luau> --output <json>");
