...
const report = {
  schemaVersion: 1,
  benchmark: "symbollattice-groovy-compiler-approved-self-recursion-v1",
  ...
  oracle: {
    ...
    contract: [
      ...
      "implicit-this self call outside closures",
      "fixed arity match",
      "no nested brace, metaclass, shadow, assignment, or static import collision"
    ]
  },
  corpora,
  baseline: { version: "0.496.0", commit: "01b4c5fdcad423e889bab411f2c2b86c0795504f", tp: 0, fn: totals.positives },
  score: { ...totals, tn: negative.tn, negatives: negative.total, negativeFalsePositives: negative.falsePositives },
  negative,
  passed:
    totals.positives > 0 &&
...
function parseArguments(args) {
  ...
  return { java: resolve(java), classpath, output, groovyVersion, corpora };
}

function scoreCorpus(name, root) {
  const truth = JSON.parse(execFileSync(options.java, [
    "-cp", options.classpath, "groovy.ui.GroovyMain", oraclePath, root
  ], { encoding: "utf8", maxBuffer: 64 * 1024 * 1024 }));
  const truthKeys = new Map(truth.candidates.map((candidate) => [candidateKey(candidate), candidate]));
  const productEdges = [];
  for (const absolute of walkGroovyFiles(root)) {
    ...
    for (const edge of facts.edges.filter((candidate) => candidate.kind === "calls")) {
  ...
  for (const [key, candidate] of truthKeys) {
    ...
    else if (validExactSelfEdge(edge)) tp += 1;
    else evidenceInvalid += 1;
  }
  const falsePositives = productEdges
    .filter(({ filePath, edge }) => !truthKeys.has(edgeKey(filePath, edge)))
  ...
  return {
    ...
    score: {
      positives: truth.candidates.length,
      tp,
...
function validExactSelfEdge(edge) {
  return edge.sourceId === edge.targetId && edge.resolution === "exact" && edge.confidence === 1 &&
    edge.evidence?.ruleId === "syntax.groovy.same-file.unique-direct-self-call.arity" &&
...
function scoreNegatives() {
  ...
  cases.forEach((testCase, index) => {
    ...
    if (facts.edges.some((edge) => edge.kind === "calls")) falsePositives.push({ index, category: testCase.category });
  });
