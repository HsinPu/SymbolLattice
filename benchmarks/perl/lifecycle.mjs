...
export function validateDisposablePerlProject(projectPath, probePath) {
  const project = resolve(projectPath);
  const probe = resolve(project, probePath);
  const relativeProbe = relative(project, probe);
  if (!basename(project).startsWith("perl-v0436-") || !existsSync(join(project, ".SymbolLattice")) || isAbsolute(relativeProbe) || relativeProbe.startsWith("..") || !probe.toLowerCase().endsWith(".pm")) {
    throw new Error("Lifecycle mutations require an indexed perl-v0436-* disposable project and an in-project Perl probe.");
  }
...
export function runPerlLifecycle({ projectPath, probePath }) {
  const { project, probe } = validateDisposablePerlProject(projectPath, probePath);
  const original = readFileSync(probe);
  const originalHash = sha256(original);
  const renamed = join(dirname(probe), `${basename(probe, ".pm")}.renamed.pm`);
  const deleted = join(dirname(project), `${basename(project)}-deleted-probe.pm`);
  ...
  try {
    ...
    writeFileSync(probe, Buffer.concat([original, Buffer.from("\n\npackage Lifecycle::Added;\nsub added { 1 }\n", "utf8")]));
    const semantic = runCli("sync", project);
    if (semantic.counts.symbols !== baseline.counts.symbols + 2 || semantic.counts.edges !== baseline.counts.edges + 2) throw new Error("Semantic mutation did not add exactly one Perl package and one sub with containment.");
    ...
    if (removal.counts.files !== baseline.counts.files - 1) throw new Error("Delete did not remove exactly one indexed Perl file.");
    ...
    if (!sameCounts(restore.counts, baseline.counts) || reopen.generationId !== restore.generationId || !sameCounts(reopen.counts, baseline.counts)) throw new Error("Restore or fresh-process reopen did not retain baseline Perl graph.");
    ...
    return { schemaVersion: 1, benchmark: "symbollattice-perl-large-project-lifecycle-v1", project, probePath, originalSha256: originalHash, evidence, acceptance: { status: "pass" } };
  } finally {
...
if (process.argv[1] !== undefined && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if ([projectPath, probePath, output].some((value) => value === null)) throw new Error("Usage: --project <disposable project> --probe <relative.pm> --output <json>");
  const report = runPerlLifecycle({ projectPath, probePath });
