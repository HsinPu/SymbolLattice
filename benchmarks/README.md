...
For an unhinted natural-language rejection question, an exact CommonJS property reference from a selected production callable to an exported error variable can place the reference site and error definition first when they share a query term. The optional `queryPlan.rejectionReferencePriority` names both symbols, their previous ranks, shared terms, and the exact edge IDs. Its evidence scope is explicitly `static-property-reference`: the edge does not establish an executed call or full runtime path. Explicit file requests and numeric queries retain their existing ordering. This additive query-plan receipt is a minor-version change; the index format and install contract do not change.

On pinned [Fastify](https://github.com/fastify/fastify) `70b14e92c0b55e8201f5530ba2e6bab4e928c784`, the two development questions in `fastify-unhinted-content-type-tasks.json` now put `lib/contentTypeParser.js` and `lib/errors.js` first. For the unsupported-content-type question they previously appeared at focus ranks 5 and 4; for the unknown-media-type question, at 3 and 2. Each new receipt was checked against a returned exact `references` edge at `lib/contentTypeParser.js:184` with a CommonJS binding path to `lib/errors.js:111`. All ten specified source facts across the two tasks remain present. The unknown-media-type result still includes `lib/validation.js` and `lib/schemas.js`: its judged file precision remains 2/4, so this changes first-read order rather than removing false positives.

Across 20 fixed manifests and 27 tasks on pinned Django, NestJS, and Fastify indexes, all required files and specified source facts remained available. Only those two Fastify tasks changed selected-file order; every task's selected-file set was unchanged. These are known development/regression cases, not held-out evidence of a general ranking gain. The one fresh CLI process per task includes startup and freshness work, so it does not establish a query-speed improvement or regression. First indexing, incremental sync, peak memory, and Agent completion time were not measured. Raw reports are `%TEMP%/SymbolLattice-evidence-062500-retrieval/*.json`; reproduce each with `node benchmarks/mcp/task-retrieval.mjs --project <matching-pinned-indexed-checkout> --manifest benchmarks/mcp/<matching-task-manifest>.json --output <external-report.json> --repetitions 1`.

## v0.528.26 bounded UTF-8 freshness hashing

Native full-content freshness checks now buffer known-language UTF-8 source files up to 1 MiB, reading at most one extra byte to detect growth. Valid UTF-8 without a leading BOM has the same SHA-256 whether hashed as bytes or as decoded text; BOM and invalid UTF-8 still use the decoded-text identity from initial indexing. Files larger than the cap, or growing through it while being read, use the previous streaming decoder. At most 32 source reads run together. Shell/Lua raw-byte hashing, content-classified source reads, injected readers, configuration hashing, source selection, and freshness outcome rules are unchanged. No index rebuild or query contract change is required; this is a patch performance correction. The buffer limit bounds each small-file read, though peak process memory was not measured.

On Windows / Node v24.19.0, the v0.528.25 built store (`01f29c5`) and candidate were compared on pinned [Django](https://github.com/django/django) `bc833e8883db4a333a6485d91637b78c85e2b13b`, [NestJS](https://github.com/nestjs/nest) `35c3ded6dbf3f23f917ae88d0ed966932788cae6`, and [Fastify](https://github.com/fastify/fastify) `70b14e92c0b55e8201f5530ba2e6bab4e928c784`. Their complete ordered source-fingerprint arrays were deeply equal, covering 3,366, 1,738, and 338 files respectively. Six alternating paired full-content freshness checks after two warmup pairs all reported `proven-unchanged` and checked every indexed file. Median total/source-hash time (ms, before → after) was Django 819.59/432.40 → 811.67/422.50, NestJS 345.56/208.40 → 324.55/200.90, and Fastify 77.73/61.90 → 63.53/46.50. These are fixed warm checks; Django's full-check gain is much smaller than the isolated hashing prototype's gain. The prototype comparisons are not attributed to the shipped implementation.

Three alternating paired persistent-service queries per project returned deeply equal complete results, including selected files, source, graph receipts, freshness, and truncation metadata. Query medians were Django atomic rollback 2,210 → 2,124 ms, Fastify unknown media type 715 → 697 ms, and NestJS constructor dependencies 1,644 → 1,536 ms. Three pairs do not establish a reliable query-speed improvement or general Agent task-time gain. The four relevance false positives fixed as truth in v0.528.25 remain; this change preserves evidence identity but does not improve ranking. First indexing, changed-file sync, and peak memory were not measured. Raw reports are `%TEMP%/SymbolLattice-evidence-061000-{django,nest,fastify}-{fingerprints-equal,freshness-paired,explore-paired}.json`. Reproduce full freshness with `node benchmarks/filesystem/freshness-verify.mjs --project <pinned-indexed-checkout> --baseline-product-root <v0.528.25-built-root> --candidate-product-root <v0.528.26-built-root> --output <external-report.json> --repetitions 6`; use `benchmarks/mcp/paired-explore.mjs` with matching roots for query comparison. `npm run check`, `npm run build`, and the full `npm test -- --maxWorkers 2` passed (3,227 tests passed, four existing skips).

## v0.528.25 adjudicated retrieval false positives

Four previously unjudged returned files were reviewed against pinned source and added as explicit irrelevant-file judgments in two existing development tasks. This is a benchmark truth expansion, not a retrieval change or held-out validation: the files were chosen after seeing earlier output, then checked against source independently. For Django's atomic exception/savepoint question, `django/db/backends/base/creation.py:148–167` uses an atomic block to deserialize test data, while `django/contrib/postgres/operations.py:112–117` forbids certain schema operations inside a transaction. Neither implements the requested exception branch or connection savepoint rollback. For Fastify's unknown-media-type rejection question, `lib/handleRequest.js:47–51` enters `contentTypeParser.run`; its undefined-parser branch in `lib/contentTypeParser.js:175–186` sends an error and returns before `lib/validation.js` runs at `lib/handleRequest.js:85`. `lib/schemas.js:150–190` selects response schemas by media type, not the request parser rejection. The other returned files in these manifests retain their previous judgments.

Single fresh-process v0.528.24 runs on Django `bc833e8883db4a333a6485d91637b78c85e2b13b` and Fastify `70b14e92c0b55e8201f5530ba2e6bab4e928c784` each retained both required files and all five specified source facts in the affected task, but also returned both newly judged irrelevant files. Each affected task therefore has 2 TP, 2 FP, 0 FN and judged precision 2/4 within its fully judged four-file result. Fastify's separate unsupported-content-type task still has 2 TP, 0 FP and two unjudged files. These task-local figures do not establish corpus-wide precision. Raw reports are `%TEMP%/SymbolLattice-evidence-060900-atomic-baseline.json` and `%TEMP%/SymbolLattice-evidence-060900-media-baseline.json`; reproduce with `node benchmarks/mcp/task-retrieval.mjs --project <matching-pinned-indexed-checkout> --manifest benchmarks/mcp/<matching-task-manifest>.json --output <external-report.json> --repetitions 1`. The product's search results are unchanged; this patch makes a known relevance gap measurable.

## v0.528.24 bounded discovery concurrency

Source and freshness discovery classify candidate files after the ignore-aware project walk. The candidate-read ceiling increased from 32 to 64, still bounding simultaneous file opens and leaving source selection, ignore rules, hashing, and freshness policy unchanged. This is a patch latency adjustment without an index or public query contract change.

On Windows / Node v24.19.0, eight alternating paired warm discovery runs compared the v0.528.23 build (`1705f8d`) with the 64-read candidate on pinned [Django](https://github.com/django/django) `bc833e8883db4a333a6485d91637b78c85e2b13b`, [NestJS](https://github.com/nestjs/nest) `35c3ded6dbf3f23f917ae88d0ed966932788cae6`, and [Fastify](https://github.com/fastify/fastify) `70b14e92c0b55e8201f5530ba2e6bab4e928c784`. Complete source and configuration path arrays were equal on every run. Upper-median discovery time changed 358.42 → 350.73 ms for Django (3,366 source paths), 92.53 → 92.36 ms for NestJS (1,738), and 14.96 → 14.38 ms for Fastify (338). All eight Django paired differences favored 64 reads, by 2.9–23.9 ms. Separate Django trials with ceilings of 16 and 128 had medians of 353.46 → 365.32 ms and 359.72 → 356.15 ms respectively; these were disposable built-file experiments, not shipped alternatives.

The six measured paired full-content freshness checks after two warmup pairs still reported `proven-unchanged` and checked all indexed files. Median total/discovery time changed 815.54/386.84 → 793.15/380.86 ms for Django, 313.63/103.31 → 326.68/105.90 ms for NestJS, and 76.89/19.27 → 75.19/18.77 ms for Fastify. A second NestJS run reversed the total-time direction (335.88 → 322.02 ms), so it does not establish an improvement or regression there. These warm, fixed-corpus timings do not establish universal speedup, first-index or incremental-sync gains, peak memory, or Agent task time. Raw reports are `%TEMP%/SymbolLattice-evidence-060800-{django,nest,fastify}-concurrency64.json`, `%TEMP%/SymbolLattice-evidence-060800-django-concurrency{16,128}.json`, `%TEMP%/SymbolLattice-evidence-060800-{django,nest,fastify}-freshness64.json`, and `%TEMP%/SymbolLattice-evidence-060800-nest-freshness64-repeat.json`. Reproduce full freshness with `node benchmarks/filesystem/freshness-verify.mjs --project <pinned-indexed-checkout> --baseline-product-root <v0.528.23-built-root> --candidate-product-root <v0.528.24-built-root> --output <external-report.json> --repetitions 6`. `npm run check`, `npm run build`, and the full `npm test -- --maxWorkers 2` passed (3,226 tests passed, four existing skips).
...
Multi-concept symbol search previously lowercased every symbol name and qualified name into a temporary table on each query. A derived `symbol_casefolds` table now stores those same SQLite-folded values beside the active symbol projection. Its generation marker is published in the same write transaction as the new projection; a reader uses it only when the marker matches the active generation inside one SQLite read snapshot. Missing or out-of-date copies fall back to the previous query. Writable initialization backfills an existing active generation, while a read-only installation keeps working through the fallback until its index is refreshed. An older writer can still replace the projection: the changed generation marker prevents the old folded copy from being used. The table is additive and the CLI/MCP output contract is unchanged, so this is a patch; users do not need to rebuild an index for correctness.

On pinned [Django](https://github.com/django/django) `bc833e8883db4a333a6485d91637b78c85e2b13b`, [Fastify](https://github.com/fastify/fastify) `70b14e92c0b55e8201f5530ba2e6bab4e928c784`, and [NestJS](https://github.com/nestjs/nest) `35c3ded6dbf3f23f917ae88d0ed966932788cae6`, eight alternating paired warm service queries against each unchanged checkout measured upper-median total time (v0.528.21 → v0.528.22) of 2,269 → 2,160 ms for the Django atomic-rollback task, 739 → 734 ms for Fastify request validation, and 1,676 → 1,670 ms for NestJS request pipe transformation. Their seed-retrieval stages measured 1,281 → 1,171 ms, 518 → 514 ms, and 1,053 → 1,039 ms respectively. Every paired run asserted complete response equality. The small total-time differences for Fastify and NestJS do not establish a reliable end-to-end gain on those queries. These are individual fixed queries, not a corpus-wide latency or Agent task-time claim. The derived table occupied 24.69 MiB, 2.32 MiB, and 7.25 MiB in those indexes. First writable initialization backfilled the copies in approximately 0.80 s, 0.09 s, and 0.22 s respectively, with active generation IDs and all four status counts unchanged. First indexing, full incremental-sync latency, peak memory, and Agent query count were not measured.

Across all 20 fixed Django/Fastify/NestJS manifests, the v0.528.22 build returned the same complete query outputs, selected files, and source, lexical, and graph verification records as v0.528.21: 27 tasks, 58 necessary-file occurrences, 113 specified source facts, and 872 verified graph edges, with zero specified misses. Unjudged returned files and corpus-wide precision remain unmeasured. Reports are under `%TEMP%/SymbolLattice-evidence-060500-regression/` (baseline) and `%TEMP%/SymbolLattice-evidence-060600-v22-regression/` (candidate); paired reports are `%TEMP%/SymbolLattice-evidence-060600-{atomic,fastify,nest}-v22-paired.json`. Reproduce a paired query with `node benchmarks/mcp/paired-explore.mjs --project <matching-pinned-indexed-checkout> --baseline-root <v0.528.21-built-root> --candidate-root <v0.528.22-built-root> --query "<same-task-query>" --output <external-report.json> --pairs 8`; reproduce task receipts with `node benchmarks/mcp/task-retrieval.mjs --project <matching-pinned-indexed-checkout> --manifest benchmarks/mcp/<matching-task-manifest>.json --output <external-report.json> --repetitions 1`. The paired test requires both builds against the same generation and an initialized candidate folded copy. `npm run check`, `npm run build`, `npm run verify:mcp-worker-generation`, and the full `npm test -- --maxWorkers 2` passed (3,226 tests passed, four existing skips).

## v0.528.21 active-generation count receipts

The bounded graph read previously ran four `COUNT(*)` queries for status on every search. A complete active generation already stores exact projection counts in its version-2 snapshot metadata, written in the same transaction as the projection and active pointer. The reader now uses that receipt when its four counts are valid nonnegative integers, and falls back to live table counts for old generations or missing/damaged metadata. An external writer's generation switch is still read inside one SQLite snapshot. This changes no search policy, output contract, or index format, so it is a patch and existing indexes do not need rebuilding.

On the pinned Django `bc833e8883db4a333a6485d91637b78c85e2b13b`, Fastify `70b14e92c0b55e8201f5530ba2e6bab4e928c784`, and NestJS `35c3ded6dbf3f23f917ae88d0ed966932788cae6` indexes, saved counts exactly matched all four live table counts. Across 24 alternating warm reads, the median count-read cost was 31.47 → 0.07 ms for Django, 0.17 → 0.05 ms for Fastify, and 12.02 → 0.08 ms for NestJS. Eight alternating paired persistent-service reads of the Django atomic-rollback query, after warmup, measured median total time 2,295 → 2,236 ms and seed retrieval 1,288 → 1,254 ms. These measurements cover existing indexes on Windows/Node v24.19.0; they do not establish a general speedup for every query, first indexing, incremental sync, peak memory, or Agent completion time.

The 20 fixed manifests over those three pinned projects returned identical full query results, selected files, and source, lexical, and graph verification records before and after the change: 27 tasks, 58 necessary-file occurrences, 113 specified source facts, and 872 graph receipts, with zero specified misses. These are fixed development tasks; unjudged files and corpus-wide precision remain unmeasured. Raw reports are under `%TEMP%/SymbolLattice-evidence-060300-final-regression/` (baseline) and `%TEMP%/SymbolLattice-evidence-060500-regression/` (candidate); paired times are in `%TEMP%/SymbolLattice-evidence-060500-atomic-paired.json`. Reproduce a task with `node benchmarks/mcp/task-retrieval.mjs --project <matching-pinned-indexed-checkout> --manifest benchmarks/mcp/<matching-task-manifest>.json --output <external-report.json> --repetitions 1`. `npm run check`, `npm run build`, and the complete `npm test -- --maxWorkers 2` passed (3,225 tests passed, four existing skips).

## v0.528.20 bounded rollback query and named-method evidence

Long natural-language queries still admit at most eight identifier terms (twelve for numeric queries). The planner now treats “roll back” as the `rollback` identifier concept, omits the pronoun “its”, and, only when the term budget would be exceeded, omits a generic “handle a/an/the” verb and “raised” when “exception” is also present. It keeps the original clause order and does not replace the first eight terms with uniform sampling. Within a file already selected for the query, one or two generic same-class method excerpts may yield to an exact query-name method and a related compound-name method when both have literal source support, meet the relative score floor, and preserve every previously covered query term. Existing source-gap, property-use, numeric, and proven flow selections remain protected. Method-name agreement is relevance evidence, not proof of a call relationship. Policy `explore-query-plan-v28` identifies the selection change. This corrects the existing query behavior without an index-format or public command change, so it is a patch and no reindex is required.

On pinned [Django](https://github.com/django/django) `bc833e8883db4a333a6485d91637b78c85e2b13b`, the independently fixed atomic rollback task improves from 1/2 required files and 0/5 specified source facts in v0.528.19 to 2/2 files and 5/5 facts. The returned excerpts include `Atomic.__exit__` at `django/db/transaction.py:283,295` and `BaseDatabaseWrapper.rollback` / `savepoint_rollback` at `django/db/backends/base/base.py:333,405,413`. The harness verified the emitted source and graph receipts against that pinned checkout. Two other selected files remain unjudged; this case does not establish repository-wide precision or general Python call resolution. The other 26 fixed Django/Fastify/NestJS tasks retain all 43 required file occurrences and 108 specified source facts. Their selected files, focuses, source windows, connections, evidence paths, source allocations, query terms, and response byte counts match the prior reports; the overall policy and product version change. The final 20-manifest / 27-task run has zero required-file and fixed-source-fact misses.

Eight alternating paired persistent-service reads of the atomic query after warmup measured median elapsed time 2,255 → 2,336 ms, seed retrieval 1,248 → 1,307 ms, and planning 79 → 96 ms. This measured query costs about 81 ms more while returning the missing evidence; it is not a speed improvement. A separate eight-pair run of the unchanged Django middleware task measured 2,378 → 2,342 ms total, 1,377 → 1,380 ms seed retrieval, and 77 → 75 ms planning; these small differences do not establish an improvement on unchanged queries either. The baseline build is the v0.528.18 snapshot with the same retrieval behavior as documentation-only v0.528.19. Single fresh-process timings for the 26 unchanged tasks were faster in 14 cases and slower in 12, with a median paired difference of about −2 ms; one sample per task does not establish a latency trend. First indexing, incremental sync, peak memory, and Agent task completion time/query count were not measured. Raw task reports are under `%TEMP%/SymbolLattice-evidence-060300-final-regression/`; prior reports are under `%TEMP%/SymbolLattice-evidence-060100-candidate-tasks/` and `%TEMP%/SymbolLattice-evidence-060200-atomic-v19.json`. Paired stage timings are in `%TEMP%/SymbolLattice-evidence-060300-{planning-paired,unchanged-planning-paired}.json`. Reproduce retrieval with `node benchmarks/mcp/task-retrieval.mjs --project <matching-pinned-indexed-checkout> --manifest benchmarks/mcp/<matching-task-manifest>.json --output <external-report.json> --repetitions 1`. `npm run check`, `npm run build`, and the complete `npm test -- --maxWorkers 2` passed (3,225 tests passed, four existing skips). The external corpora, indexes, and generated reports remain outside the repository.
...
Reproduce the v0.528.19 miss with `node benchmarks/mcp/task-retrieval.mjs --project <pinned-indexed-Django-checkout> --manifest benchmarks/mcp/django-atomic-rollback-tasks.json --product-root <v0.528.19-built-root> --output <external-report.json> --repetitions 1`. That patch added fixed regression truth and documentation only; it did not change the query or index contract. Existing indexes did not need rebuilding.

The v0.528.20 result above closes the five cited source-fact misses for this fixed query. The v0.528.19 figures remain as the pre-fix baseline.

## v0.528.18 bounded source-candidate discovery

The shared scoped walker now applies ignore and scope rules before checking source candidates in batches of at most 32. Previously it awaited each candidate in a directory, including a bounded shebang read for every unclassified path. Source admission rules, full-content hashes, and missing/unreadable-path handling are unchanged. The index format and public query contract are unchanged, so this is a patch; existing indexes do not need rebuilding.

On Windows with Node v24.19.0, six measured full-content freshness checks per build, after two warmup pairs and alternating v0.528.17/v0.528.18 order, had median times of 996/807 ms on pinned [Django](https://github.com/django/django) `bc833e8883db4a333a6485d91637b78c85e2b13b` (3,366 indexed files), 336/315 ms on [NestJS](https://github.com/nestjs/nest) `35c3ded6dbf3f23f917ae88d0ed966932788cae6` (1,738 files), and 78/74 ms on [Fastify](https://github.com/fastify/fastify) `70b14e92c0b55e8201f5530ba2e6bab4e928c784` (338 files). The Django discovery subphase median fell from 552 to 397 ms. All checks reported `proven-unchanged` and checked the complete indexed file set. These isolated samples do not guarantee a general speedup, and first indexing, changed-file sync, peak memory, and Agent completion time were not measured.

On those same indexed checkouts, 19 fixed manifests / 26 tasks produced 26/26 identical full query responses between builds. All 43 required file occurrences and 108 fixed source facts were returned; 853 task-local graph-edge receipts passed the existing verifier. One fresh CLI process per task was faster on 18/26 tasks, with median paired difference −16 ms (range −392 to +45 ms); these single task samples are descriptive, not a general latency claim. The task reports are under `%TEMP%/SymbolLattice-evidence-060100-{baseline,candidate}-tasks/`; paired freshness reports are `%TEMP%/SymbolLattice-evidence-060100-freshness-{django,nest,fastify}.json`. Reproduce freshness with `node benchmarks/filesystem/freshness-verify.mjs --project <pinned-indexed-checkout> --baseline-product-root <v0.528.17-built-root> --candidate-product-root <v0.528.18-built-root> --output <external-report.json> --repetitions 6`; reproduce retrieval with `node benchmarks/mcp/task-retrieval.mjs --project <pinned-indexed-checkout> --manifest benchmarks/mcp/<matching-task-manifest>.json --product-root <built-product-root> --output <external-report.json> --repetitions 1` for both builds. `npm run check`, `npm run build`, the focused walker/shebang/source-catalog tests, and the complete `npm test -- --maxWorkers 2` passed (3,223 tests passed, four existing skips).

## v0.528.17 source-gap promotion evidence

Literal source-search receipts still include comments and strings, but a file can no longer be promoted solely to fill a missing query concept when its nearby support appears only on obvious whole-line `//` comments or Python `#` comments. Promotion now needs two nearby non-comment-line concepts and a weighted missing-concept score of at least 0.2 among the bounded eligible candidates. Inline and block comments are not classified by this guard; strings remain literal evidence, not proof of execution. The frequency weight is scoped to the bounded candidate population, not repository-wide rarity. `explore-query-plan-v27` identifies this ranking correction. The index format, query interface, and existing source receipts are unchanged, so this is a patch and no reindex is required when upgrading from v0.528.16.

On pinned [Django](https://github.com/django/django) `bc833e8883db4a333a6485d91637b78c85e2b13b`, [Fastify](https://github.com/fastify/fastify) `70b14e92c0b55e8201f5530ba2e6bab4e928c784`, and [NestJS](https://github.com/nestjs/nest) `35c3ded6dbf3f23f917ae88d0ed966932788cae6`, the 19 existing manifests covered 26 tasks. All 43 required file occurrences and 108 independently specified source facts were retained; the verifier accepted 853 task-local graph-edge receipts, 72 connections, 41 directed path steps, and 373 reverse-impact steps. Three selected-file sets changed. Manual inspection of the pinned sources found that `django/http/multipartparser.py:119` uses “exceptions are thrown” in a parser-cleanup comment, and `lib/schemas.js:26` uses “instead” in a schema comment; neither comment now justifies a source-gap promotion. The NestJS shutdown task no longer uses the generic “events” hit in `packages/microservices/server/server-tcp.ts:181`, which appeared in 30 eligible candidates. The source-backed Fastify `lib/errors.js:111-113` “Unsupported Media Type” case remains selected with its literal source receipt. Other candidate files still occupy the vacated result slots, so these observations do not establish repository-wide precision improvement or complete removal of irrelevant files. The Django middleware response shrank from 868,201 to 736,150 JSON bytes; the Fastify cookie response grew from 349,665 to 386,510 bytes.

Two alternating six-read persistent-connection samples of the Django middleware query measured baseline medians of 1,348 and 1,356 ms versus 1,354 and 1,342 ms with this change. The results do not establish a speed improvement or regression. One fresh CLI process per task was faster on 19 of 26 tasks, but the single samples are too weak for a general latency claim; indexing, peak memory, and Agent completion time or query count were not measured. The fixed indexes were synchronized to the current extractor before comparison. Raw task reports are under `%TEMP%/SymbolLattice-evidence-055500-{baseline,candidate}-tasks/`, and isolated read reports under `%TEMP%/SymbolLattice-evidence-055500-bounded-*.json`. Reproduce with `node benchmarks/mcp/task-retrieval.mjs --project <pinned-indexed-checkout> --manifest benchmarks/mcp/<matching-task-manifest>.json --output <external-report.json> --repetitions 1 --product-root <built-product-root>` and `node --expose-gc benchmarks/mcp/bounded-graph-read.mjs --project <pinned-indexed-checkout> --query "How is synchronous request middleware assembled so thrown exceptions become responses?" --output <external-report.json> --repetitions 6 --product-root <built-product-root>`. `npm run check`, `npm run build`, `npm run verify:mcp-worker-generation`, and the full `npm test -- --maxWorkers 2` passed (3,221 tests passed, four existing skips).

## v0.528.16 bounded symbol ranking temp storage

The repository-wide, multi-concept symbol candidate read now uses SQLite memory-backed temporary storage only while ranking an index with 8,192–65,536 symbols, then restores the connection default. The second, file-scoped symbol read and other query stages keep their previous storage setting. This row-count guard limits when the extra memory can be used, but does not impose a strict byte cap. The SQL, candidate order, evidence, index format, and external query contract are unchanged; existing indexes do not need rebuilding. This is a patch-level query-cost improvement.

On pinned [Django](https://github.com/django/django) `bc833e8883db4a333a6485d91637b78c85e2b13b` (58,495 indexed symbols), all four existing request-error and exact-binding tasks produced structurally identical full responses between v0.528.15 and v0.528.16. They retained 7/7 required file occurrences, 20/20 independently specified source facts, and the verifier accepted 87 task-local graph-edge receipts. Two alternating fresh-CLI samples per task found both candidate samples faster for each of the three natural-language tasks; the exact binding lookup was mixed. For example, the synchronous middleware task took 4,839 and 4,891 ms before, versus 4,747 and 4,737 ms after. These small samples include startup, freshness checks, query work, and serialization, and do not establish a general latency guarantee.

A separate six-read persistent-connection measurement of that middleware query returned the same bounded graph response hash with both builds (3,157 returned symbols, 3,539 edges). Separate serial runs measured 1,477 ms median before and 1,337 ms with the final build for the synchronous graph read. Single-process post-GC memory readings varied between runs and do not bound peak or eight-worker pool memory. First indexing, incremental sync, and Agent task time or query count were not measured. Raw task reports are under `%TEMP%/SymbolLattice-evidence-055400-temp-store-tasks/` and graph-read reports under `%TEMP%/SymbolLattice-evidence-055400-bounded-*.json`. Reproduce task and source verification with `node benchmarks/mcp/task-retrieval.mjs --project <pinned-indexed-checkout> --manifest benchmarks/mcp/django-request-errors-tasks.json --output <external-report.json> --repetitions 2 --product-root <built-product-root>` (repeat with `django-module-bindings-tasks.json`), and the isolated read with `node --expose-gc benchmarks/mcp/bounded-graph-read.mjs --project <pinned-indexed-checkout> --query "How is synchronous request middleware assembled so thrown exceptions become responses?" --output <external-report.json> --repetitions 6 --product-root <built-product-root>`. `npm run check`, `npm run build`, `npm run verify:mcp-worker-generation`, and the full `npm test -- --maxWorkers 2` passed (3,220 tests passed, four existing skips).
...
The bounded graph reader now joins the opposite symbol endpoint only: every frontier ID was already read from `symbols` or admitted through a previously joined edge. This removes a redundant lookup without admitting orphan edges or changing traversal bounds. The task-retrieval verifier also checks emitted graph-edge paths and line/column coordinates against pinned source, exact-edge rule provenance, direct connection endpoints, directed path steps, and reverse-impact steps. Valid coordinates and consistent endpoints do not establish that a resolver chose the semantically correct target; that remains a separate oracle question.

On pinned [Fastify](https://github.com/fastify/fastify) `70b14e92c0b55e8201f5530ba2e6bab4e928c784`, [NestJS](https://github.com/nestjs/nest) `35c3ded6dbf3f23f917ae88d0ed966932788cae6`, and [Django](https://github.com/django/django) `bc833e8883db4a333a6485d91637b78c85e2b13b`, all 26 tasks in the 19 existing manifests returned the same JSON result as the fixed pre-change task reports: 43/43 required file occurrences and 108/108 independently specified source facts. The new verifier checked 854 task-local distinct edge receipts, 70 direct connections, 41 directed path steps, and 370 reverse-impact steps against those pinned checkouts; repeated edges across tasks are counted again. File judgments outside each manifest remain incomplete.

Paired local SQLite measurements on the final v0.528.14 build alternated the two SQL forms six times on each captured frontier batch. Summed mean edge-read time for one Fastify unsupported-content-type query was 108.719 → 94.823 ms, for one NestJS constructor-dependency query 306.425 → 291.730 ms, and for one Django synchronous-middleware query 162.039 → 140.953 ms. Every batch returned the same projected rows. These figures isolate edge reads and do not measure complete Agent tasks. Separate 30-request persistent MCP samples (pool size one, three warmups, source-session deduplication disabled) gave Fastify mean 598.450 and 610.599 ms for the two-join build versus 596.867 ms for the single-join build, and NestJS 1,960.239 versus 1,867.952 ms. The two MCP builds differed only in the SQL joins and were made before the version-string bump. Django was order-sensitive: single-join 3,672.938 ms, then two-join 3,543.196 ms, then single-join 3,534.053 ms. This supports a lower isolated edge-read cost, not a general end-to-end speed guarantee or a Django latency improvement. First indexing, incremental sync, Agent completion time, and query count were not measured.

The task reports are under `%TEMP%/SymbolLattice-evidence-055200-single-join-tasks/`, edge-read reports are `%TEMP%/SymbolLattice-evidence-055200-*-edge-lookup.json`, and the 30-request latency reports are `%TEMP%/SymbolLattice-evidence-055200-*-mcp*.json`. Reproduce source-truth and result checks with `node benchmarks/mcp/task-retrieval.mjs --project <pinned-indexed-checkout> --manifest benchmarks/mcp/<task-manifest>.json --output <external-report.json> --repetitions 1`; isolated SQL reads with `node benchmarks/mcp/edge-lookup.mjs --project <pinned-indexed-checkout> --query "<task query>" --output <external-report.json>`; and persistent latency with `node benchmarks/mcp/explore-latency.mjs --project <pinned-indexed-checkout> --cli dist/cli/main.js --output <external-report.json>`. `npm run check`, `npm run build`, `npm run verify:mcp-worker-generation`, and the full `npm test -- --maxWorkers 2` passed (3,218 tests passed, four existing skips).

## v0.528.13 persistent MCP read-page cache

The worker-local persistent read-only SQLite connection now uses a 16 MiB page-cache target. One-shot CLI readers keep the SQLite default. This is a patch-level query-cost change: graph traversal, generation fencing, freshness checks, source evidence, and the index format are unchanged. Each MCP worker has its own cache; the pool permits at most eight workers, so memory cost can grow with pool size.

On pinned [Fastify](https://github.com/fastify/fastify) `70b14e92c0b55e8201f5530ba2e6bab4e928c784`, two sequential baseline/candidate comparisons with reversed run order used 30 persistent MCP requests per build, pool size one, source-session deduplication disabled, and the same three exploratory queries. Mean latency was 625.036 → 600.708 ms and 624.892 → 597.230 ms; p50 was 599.080 → 578.990 ms and 610.726 → 571.648 ms. The pinned [NestJS](https://github.com/nestjs/nest) `35c3ded6dbf3f23f917ae88d0ed966932788cae6` constructor-dependency task measured 2,122.407 → 2,066.608 ms mean across 30 requests. On pinned [Django](https://github.com/django/django) `bc833e8883db4a333a6485d91637b78c85e2b13b`, the middleware task measured 4,201.999 → 4,156.832 ms mean, but p50 rose 3,921.360 → 3,929.505 ms and p95 rose 5,374.010 → 5,471.408 ms. These samples support faster mean requests on the tested queries, not a general latency guarantee or a Django tail-latency improvement.

Four Fastify MCP responses and one response each from NestJS and Django were byte-identical between builds. A separate 30-read Fastify process with forced GC observed final RSS of 201,588,736 → 211,640,320 bytes; this single-process sample does not bound peak memory in a full MCP pool. `npm run verify:mcp-worker-generation` confirmed that one worker read a newly published generation without fallback after sync. `npm run check`, `npm run build`, and the full `npm test -- --maxWorkers 2` passed (3,217 tests passed, four existing skips). First indexing, incremental sync latency, Agent task time, and query count were not measured. Reproduce persistent MCP latency with `node benchmarks/mcp/explore-latency.mjs --project <pinned-indexed-checkout> --cli <built-cli-path> --output <external-report.json>`; Fastify reports are under `%TEMP%/SymbolLattice-evidence-055100-mcp-*.json`, with NestJS and Django reports under `%TEMP%/SymbolLattice-evidence-055100-*-mcp-*.json`.
