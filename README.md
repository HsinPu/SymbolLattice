...
> [!IMPORTANT]
> v0.316.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 明示的 `this.method()`、`super.method()`、具有直接 reference type 的 parameter／block-local／field receiver，以及 `var receiver = new DirectType(...)`，會重用相同的 overload、method-set 與 access 證據。欄位支援裸名稱、`this.field`、`super.field`，以及由明確 import、同 package 或完整限定名稱證明的 `TypeName.FIELD.method()`。型別限定 lookup 只接受 static field，可沿 exact class／interface 圖處理父類欄位、介面常數、hiding、class precedence 與歧義；public、同 package 連續繼承路徑及宣告類別 private-self 可解析，跨 package protected qualifier 暫時保持不解析。收據保存限定 owner 的 class／interface 種類、實際欄位 owner、完整路徑、modifier、access 與 bounds。區域或 caller 階層的同名值遮蔽、instance field hiding、不可存取／未知型別、循環、多重 owner、界限與 static context 違規都 fail closed。`var` 只接受非泛型直接 object creation；nestmate private、一般賦值、泛型／array 欄位與 classpath 推測仍保持不解析。`super.method()` 仍要求唯一 exact 直接父類並保留完整路徑。
