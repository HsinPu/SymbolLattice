...
> [!IMPORTANT]
> v0.290.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- `explore` 同時支援精確 symbol 與有界問題模式；明示的安全專案相對路徑優先，其餘焦點候選會把 symbol／檔名證據、候選間連線與有界 exact one-hop graph mass 乘上 persisted generated-source worth（`0.3`）及 test-source worth（`0.5`）。一般問題只有在至少 2 個不同候選檔已被同一 generation 證明為 production 時，才排除未明示的 test 候選；證據不足時仍只柔性降權，測試意圖與明示測試檔一律保留。receipt 會揭露觸發原因、候選／排除計數與最多 16 個被排除檔案的來源角色證據。最多選取 4 個檔案、8 個焦點、每檔 2 個焦點與 16 條連線；問題模式只用 exact 關係尋找 4 hops 內的路徑。
