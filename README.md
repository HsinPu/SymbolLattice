...
> [!IMPORTANT]
> **v0.46.0** is an early developer release. This public repository runs from source; its npm package is intentionally private and is not published to npm.
...
| Capability | Proven surfaces in v0.46 |
| --- | --- |
...
| Play | Scala direct `conf/routes` / `conf/*.routes` literal controller-action entries with exact unique package-class-method handlers or explicit unresolved evidence |
...
- One direct Scala package clause matches `controllers`.
- Exactly one matching class/object symbol exists in that package.
- Exactly one direct body `def health` belongs to that symbol.

The resolver records every class and method candidate in evidence. If any proof is absent or ambiguous, the edge stays `unresolved` with `framework.play.conf-routes.literal-controller-action.unresolved-handler`; it never falls back to a project-wide name guess.

It intentionally excludes `->` included routers, nonliteral or malformed route rows, route entries outside `conf/routes` / `conf/*.routes`, route prefixes/composition, `build.sbt` project detection, imported or classpath controller resolution, overloads, custom route binders, reverse routing, and runtime Play behavior. Scala files with syntax errors retain only their file symbol until repaired.
...
- The Scala surface proves direct top-level classes, objects, traits, methods, and functions through the Scala AST. The Play surface proves an `exact` handler only for literal `conf/routes` / `conf/*.routes` entries whose fully static controller action has one direct matching package clause, class/object, and body method; every other accepted entry remains `unresolved`. It excludes `->` includes, prefixes/composition, `build.sbt` detection, imported/classpath controller resolution, overload resolution, custom binders, reverse routing, Scala 3 contextual declarations, generic Scala call/type analysis, and runtime Play behavior. Syntax-error Scala files retain only their file symbol until repaired.
