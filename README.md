...
> [!IMPORTANT]
> v0.182.0 是開發者預覽版，尚未發佈至 npm；請由原始碼執行。
...
- Spring Web 現支援直接 Kotlin 控制器：可證明的 `@RestController`／`@Controller` 類別，加上一個類別層級 `@RequestMapping` 與直接具體函式上的 HTTP 方法註解。
- 支援 GET、POST、PUT、PATCH、DELETE 的裸註解、單一位置字串、`path =` 與 `value =` 純字串；每條路由都以 `exact` 證據連到本地處理函式。
- 註解必須是精確直接 import 或完整 Spring 名稱；別名、萬用字元、動態或多屬性路徑、Kotlin `object`、抽象函式與方法層級 `@RequestMapping` 都會被保守排除。
...
- Spring Web 支援直接頂層、具體 Kotlin 控制器與直接具體函式上的字面 HTTP 方法註解；不推論執行期路由、條件、代理、巢狀宣告或方法層級 `@RequestMapping`。
