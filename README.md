...
**Evidence-first local code intelligence for TypeScript, JavaScript, ArkTS/ArkUI, Vue, Svelte, Astro, Razor/Blazor, Terraform/OpenTofu, Shopify Liquid, Twig, Laravel Blade, Solidity, CFML/CFScript, Nix, VB.NET, Python/Django, Go, Rust, Java/Spring/Micronaut/Jakarta REST, PHP, C, COBOL, Zig, YAML, XML/MyBatis, Java properties, Shell/Bash, SQL DDL, GraphQL schema, Protocol Buffers, Lua/Luau, Pascal, Objective-C/Objective-C++, R, Elixir, Erlang, Clojure, Perl, Julia, Haskell, OCaml, F#, Nim, C++, C#, Ruby, Kotlin, Swift, Dart, and Scala projects.**
...
> [!IMPORTANT]
> **v0.99.0** is an early developer release. This public repository runs from source; its npm package is intentionally private and is not published to npm.

> [!NOTE]
> **v0.99.0 — bounded Jakarta REST/JAX-RS route evidence.** Direct `@Path` resource classes plus one direct HTTP request-method annotation become exact local Java routes; dynamic and ambiguous annotation forms stay outside the graph.
...
- **Spring Boot property-reference evidence** - a narrowly proven direct Java field `@Value` annotation can point to one unique conventional `application` / `bootstrap` properties key with exact `references` evidence; missing and duplicate candidates stay explicit unresolved facts without guessing active profiles or configuration precedence.
- **Micronaut Controller evidence** - a direct imported or fully-qualified Java Micronaut `@Controller` plus one literal direct HTTP method annotation creates a source-ranged local route symbol and exact `routes` edge. Wildcard imports, multiple mappings, URI arrays, metadata arguments, programmatic routes, and runtime filters stay deliberately excluded.
- **Jakarta REST / JAX-RS evidence** - a direct imported or fully-qualified `jakarta.ws.rs.Path` or legacy `javax.ws.rs.Path` resource class plus one direct imported or fully-qualified marker HTTP method annotation creates a source-ranged exact local Java route. Only one literal class `@Path`, optional one literal method `@Path`, and one local handler method are accepted; `ApplicationPath`, custom methods, parameter binding, runtime configuration, and transport behavior remain excluded.
- **Shell/Bash declaration evidence** - direct, complete top-level POSIX `name() { ... }` and Bash `function name { ... }` declarations in `.sh` / `.bash` files become auditable function symbols. Quoted/commented text, nested control-flow forms, incomplete input, and here-documents do not create symbols.
- **SQL DDL resource evidence** - complete direct `.sql` `CREATE TABLE` and `CREATE VIEW` statements with unquoted literal names become source-ranged schema `resource` symbols. Comments, quoted data, and quoted identifiers are masked; incomplete declarations and dollar-quoted routine bodies do not create schema facts.
- **GraphQL schema + NestJS resolver evidence** - direct `.graphql`, `.gql`, and `.graphqls` object, interface, input, enum, scalar, and simple union definitions become source-ranged schema symbols. A direct `@Resolver(() => User)` can additionally form a bounded heuristic class-to-unique-schema-object `references` edge; descriptions, strings, comments, extensions, operations, malformed delimiters, resolver fields, and runtime execution remain outside this slice.
- **Protocol Buffers declaration evidence** - direct `.proto` top-level `message`, `enum`, and `service` declarations become source-ranged `class`, `type`, and `interface` symbols. Direct semicolon-terminated `rpc Name(Request) returns (Response);` service members become contained methods; imports, fields, options, nested declarations, RPC blocks, transport, and runtime behavior remain outside this slice.
...
node dist/cli/main.js search "active_users" --project /path/to/project --language sql
node dist/cli/main.js search "User" --project /path/to/project --language graphql
node dist/cli/main.js search "Directory" --project /path/to/project --language proto
...
| Fiber v0.84 | Go Fiber v2/v3 accepts only a direct import, same-function `app := fiber.New()`, literal App/Group paths, nested literal Group prefixes, and one unshadowed same-file named package-level handler. `Get`, `Post`, `Put`, `Patch`, `Delete`, `Head`, `Options`, `Trace`, `Connect`, and `All` become exact route facts; `All` is represented as `ALL`. |
| Echo v0.86 | Go Echo v4/v5 accepts only a direct import, same-function `app := echo.New()`, literal App/Group paths, nested literal Group prefixes, and one unshadowed same-file named package-level handler. `GET`, `POST`, `PUT`, `PATCH`, `DELETE`, `HEAD`, `OPTIONS`, and `Any` become exact route facts; `Any` is represented as `ALL`. |
...
| Spring Boot properties v0.92 | A direct Java class field with an exact `org.springframework.beans.factory.annotation.Value` import (or a fully-qualified annotation) and one static literal `${key}` / `${key:default}` form becomes a `references` edge from the class to one unique key in conventional `application(-profile)?.properties` or `bootstrap(-profile)?.properties` files. Zero or multiple candidates remain explicit unresolved evidence; values, profiles, YAML, Kotlin, aliases, wildcard imports, `@ConfigurationProperties`, SpEL, interpolation, and runtime precedence are excluded. |
| Shell/Bash v0.93 | `.sh` and `.bash` files become `shell` artifacts. Complete direct top-level POSIX `name() { ... }` or Bash `function name { ... }` declarations emit source-ranged `function` symbols with exact `language.shell.function.direct-top-level` containment evidence. Commands, calls, `source`, shebang-only files, zsh/fish syntax, here-documents, dynamic evaluation, and runtime semantics are excluded. |
| Micronaut v0.94 | Direct Java `io.micronaut.http.annotation.Controller` proof plus one direct imported or fully-qualified `Get` / `Post` / `Put` / `Patch` / `Delete` / `Head` / `Options` / `Trace` literal mapping emits a source-ranged exact local route. Marker annotations map to `/`; one positional literal or one `value` / method `uri` literal is allowed. Wildcard imports, `uris`, media/port/condition arguments, multiple mappings, custom annotations, Kotlin, RouteBuilder, and runtime behavior are excluded. |
| Jakarta REST / JAX-RS v0.99 | One direct imported or fully-qualified `jakarta.ws.rs.Path` or `javax.ws.rs.Path` class annotation plus exactly one direct local marker `GET` / `POST` / `PUT` / `PATCH` / `DELETE` / `HEAD` / `OPTIONS` annotation emits a source-ranged exact route. One literal class `@Path` and optional literal method `@Path` may be relative or slash-prefixed. Wildcard imports, dynamic or multiple paths/methods, `ApplicationPath`, custom methods, parameter binding, media/configuration annotations, and runtime behavior are excluded. |
| SQL DDL v0.95 | `.sql` files become `sql` artifacts. Complete direct semicolon-terminated `CREATE TABLE` (`IF NOT EXISTS`, `TEMP` / `TEMPORARY`, `LOCAL` / `GLOBAL TEMP`, or `UNLOGGED`) and `CREATE [OR REPLACE] [TEMP] VIEW` statements with unquoted literal qualified names emit source-ranged `resource` symbols with exact `language.sql.create-table.direct-ddl` or `language.sql.create-view.direct-ddl` containment evidence. Table bodies and view queries are not interpreted. |
| GraphQL schema v0.96 | `.graphql`, `.gql`, and `.graphqls` files become `graphql` artifacts. Complete direct object `type`, `interface`, `input`, `enum`, `scalar`, and simple `union Name = Member | Member` definitions become source-ranged `class`, `interface`, or `type` symbols with exact `language.graphql.<kind>.direct-definition` containment evidence. Fields, directives, extensions, and operations are not interpreted. |
| NestJS GraphQL resolver v0.97 | One AST-proven direct non-type-only named-import `@Resolver(() => User)` decorator (aliases allowed; single synchronous zero-argument expression-body identifier factory only) is retained. A resolver class gets one `references` edge only if exactly one indexed GraphQL object `type User` exists; it is `heuristic` at confidence `0.85`, while zero or multiple candidates remain target-null `unresolved` evidence with candidate IDs. |
| Protocol Buffers v0.98 | `.proto` files become `proto` artifacts. Complete direct top-level `message`, `enum`, and `service` blocks emit source-ranged `class`, `type`, and `interface` symbols with exact `language.proto.<kind>.direct-definition` containment evidence. Direct semicolon-terminated `rpc Name(Request) returns (Response);` members (including `stream` and dotted message names) become contained methods with exact `language.proto.rpc.direct-service-member` evidence. Fields, options, nested declarations, imports, RPC blocks, gRPC transport, and runtime behavior are not interpreted. |
| Source files | TypeScript, TSX, JavaScript, JSX, ArkTS/ArkUI, Vue SFC, Svelte SFC, Astro SFC, Razor/Blazor components, Terraform/OpenTofu HCL, Shopify Liquid, Twig, Laravel Blade, Solidity, CFML/CFScript, Nix, VB.NET, Python, Go, Rust, Java, PHP, C, COBOL, Zig, YAML, XML, Java properties, Shell/Bash, SQL, GraphQL, Protocol Buffers, Lua, Luau, Pascal, Objective-C / Objective-C++, R, Elixir, Erlang, Clojure, Perl, Julia, Haskell, OCaml, F#, Nim, C++, C#, Ruby, Kotlin, Swift, Dart, and Scala (`.ets`, `.vue`, `.svelte`, `.astro`, `.razor`, `.tf`, `.tfvars`, `.tofu`, `.liquid`, `.twig`, `.blade.php`, `.sol`, `.cfc`, `.cfm`, `.cfs`, `.nix`, `.vb`, `.c`, `.cbl`, `.cob`, `.cobol`, `.cpy`, `.zig`, `.yaml`, `.yml`, `.xml`, `.properties`, `.sh`, `.bash`, `.sql`, `.graphql`, `.gql`, `.graphqls`, `.proto`, `.lua`, `.luau`, `.pas`, `.dpr`, `.dpk`, `.lpr`, `.m`, `.mm`, source-proven `.h`, `.r`, `.ex`, `.exs`, `.erl`, `.clj`, `.pl`, `.pm`, `.jl`, `.hs`, `.ml`, `.fs`, `.nim`, `.cpp`, `.cc`, `.cxx`, `.hpp`, `.hh`, `.hxx`, `.cs`, `.rb`, `.kt`, `.swift`, `.dart`, `.scala`; plus Play `conf/routes` and `conf/*.routes` route tables) |
| Shell/Bash source files | `.sh` and `.bash` are discovered as `shell` source artifacts, participate in persisted source search and the existing CLI/MCP language validation, and never rely on a shebang guess. |
| SQL source files | `.sql` is discovered case-insensitively as a `sql` source artifact and participates in persisted source search plus the existing CLI/MCP language validation. |
| GraphQL source files | `.graphql`, `.gql`, and `.graphqls` are discovered case-insensitively as `graphql` source artifacts and participate in persisted source search plus the existing CLI/MCP language validation. |
| Protocol Buffers source files | `.proto` is discovered case-insensitively as a `proto` source artifact and participates in persisted source search plus the existing CLI/MCP language validation. |
...
| Django | A direct `from django.urls import path [as alias]`, one final literal top-level `urlpatterns` list, and a prior same-file top-level function handler emit exact `ALL` route evidence. Empty paths map to `/`; literal converters are preserved. |
...
| Shell/Bash symbols and relationships | Complete direct outer-script POSIX `name() { ... }` and Bash `function name { ... }` / `function name() { ... }` declarations produce source-ranged function symbols and exact `contains` edges only. |
| Shell/Bash module resolution | No generic Shell source, module, package, command-call, environment, or runtime resolver in this release. |
| Micronaut Java symbols and relationships | Direct Java Micronaut Controller classes retain their existing class/method symbols and add source-ranged HTTP `route` symbols with exact local `routes` edges only after exact annotation-import or fully-qualified-name proof. |
| Micronaut Java module resolution | No Micronaut route inheritance, bean, module, annotation-alias, package, or runtime resolver in this release. |
...
| Twig relationships | Complete direct literal `extends`, `include`, `embed`, `import`, and `from ... import` tag forms target only safe `.twig` logical names under the conventional project-local `templates/` root. Existing indexed targets receive exact `calls` edges; missing targets receive explicit unresolved evidence. |
| Twig module resolution | No Twig namespace, loader, bundle, inheritance-chain, dynamic-expression, `with`-context, macro-body, template-block, Symfony configuration, PHP, compiler, or runtime renderer resolver is claimed in this release. |
...
| Capability | Proven surfaces in v0.99 |
| --- | --- |
...
| Django | Direct `django.urls.path` import aliases plus a final literal same-file `urlpatterns` list and prior local function handlers |
| Gin | Go direct engine methods plus same-function literal `RouterGroup` prefixes |
| Fiber | Go direct Fiber v2/v3 `App` methods plus same-function literal `Group` prefixes |
| Echo | Go direct Echo v4/v5 `App` methods plus same-function literal `Group` prefixes |
...
| Micronaut | Java direct imported or fully-qualified `@Controller`, plus one literal `@Get` / `@Post` / `@Put` / `@Patch` / `@Delete` / `@Head` / `@Options` / `@Trace` direct local method; marker annotations map to `/` and only one positional or named URI literal is accepted |
| Jakarta REST / JAX-RS | Java direct imported or fully-qualified `jakarta.ws.rs.Path` or legacy `javax.ws.rs.Path`, one literal class URI template, optional one literal method `Path`, and exactly one local marker `GET` / `POST` / `PUT` / `PATCH` / `DELETE` / `HEAD` / `OPTIONS` handler annotation |
| Spring Boot properties | Direct Java class fields with a precise `@Value` import or fully-qualified annotation, one static `${key}` / `${key:default}` literal, and one unique conventional `application` / `bootstrap` `.properties` key |
...
| Horse | Pascal exactly one direct [`uses Horse;`](https://github.com/HashLoad/horse) proof, exactly one direct `program` main block, and direct main-block one-line literal `THorse.Get` / `THorse.Post` / `THorse.Put` / `THorse.Patch` / `THorse.Delete` / `THorse.Head` registrations with unique, prior same-file complete routine handlers |
...
| Nix | Complete direct returned-attribute-set, `let ... in`, and simple `inherit` declarations plus literal project-relative import syntax |
| VB.NET | Complete namespace/type/callable declaration blocks plus simple file-level `Imports` syntax |
...
v0.74 adds `.luau` discovery through the same balanced lexical declaration boundary used for Lua-compatible functions:
...
v0.85 adds `.zig` discovery and a conservative declaration pass guided by the official [Zig Language Reference](https://ziglang.org/documentation/master/). Zig files act as namespaces, and named container declarations can contain functions, so the first slice keeps only direct top-level names that can be proven without pretending to understand compiler or build-system semantics:
...
The file produces `class` symbols for direct named `struct`, `enum`, `union`, and `opaque` declarations plus `function` symbols for direct named `fn` declarations. A direct `pub` container or `pub` / `export fn` retains public visibility. Each contained declaration carries `syntax.zig.top-level-container` or `syntax.zig.top-level-function` evidence.

The scanner first masks `//` comments, quoted literals, and line-oriented `\\` multiline string literal lines while retaining source offsets, then requires balanced braces, parentheses, and brackets. It deliberately ignores imports, ordinary variables, anonymous or computed containers, `test` blocks, nested container methods, calls, aliases, `usingnamespace`, build configuration, and runtime behavior. An unbalanced file or unterminated quoted literal retains only its file symbol.

#### YAML

v0.87 adds `.yaml` and `.yml` discovery using the official [YAML 1.2.2 specification](https://yaml.org/spec/1.2.2/) and the [YAML document API](https://eemeli.org/yaml/). YAML is a data-serialization language rather than a programming-language AST, so the generic declaration slice keeps only explicit top-level scalar configuration entries from one parser-valid document:
...
This produces `variable` symbols for `service`, `port`, and `owner`; each has a direct `contains` edge with `syntax.yaml.top-level-scalar-mapping` evidence. Nested maps and sequences remain indexed source text but do not become generic derived symbols or relationships.

The parser rejects malformed and multi-document streams as a whole. Valid but outside-slice constructs such as nested mappings/sequences, aliases, anchors, explicit tags, complex keys, block scalars, imports, calls, cross-file configuration semantics, schema validation, deployment behavior, and runtime values deliberately produce no generic YAML declaration fact. This avoids treating configuration structure as executable control flow.

#### Drupal routing YAML

v0.88 adds one explicit framework exception based on Drupal's [route structure](https://www.drupal.org/docs/drupal-apis/routing-system/structure-of-routes): a parser-valid, single-document `*.routing.yml` or `*.routing.yaml` file may emit a route only when the route mapping directly proves its path and a Drupal fully qualified controller method.
...
This is deliberately not a DOM, schema, XPath, namespace, configuration, import, code-generation, or runtime model. Generic DTD-bearing input is rejected so no entity or schema semantics enter the graph. The v0.90 MyBatis mapper exception below permits one exact declaration without fetching or evaluating it. Malformed or multi-root XML also retains only its file symbol; nested grandchildren, processing instructions, comments, CDATA, and namespace declarations never create extra facts.

##### MyBatis mapper XML

v0.90 adds one deliberately narrow MyBatis 3 framework pass. It accepts a parser-valid `<mapper namespace="Java.FQN">` root with no DTD or with only the standard MyBatis mapper DTD declaration. No DTD is fetched, expanded, or used for validation.
...
#### Spring Boot `@Value` properties references

v0.92 layers a conservative cross-file Spring Boot proof over the v0.91 key facts. It accepts only a direct field inside a direct Java class when either a precise `org.springframework.beans.factory.annotation.Value` import or the fully-qualified annotation proves `@Value`, and when the annotation has one static literal `${key}` or `${key:default}` argument:

```properties
# config/application.properties
server.port=8080

# config/bootstrap-prod.properties
app.name=symbol-lattice
```
...
class AppConfig {
  ...
  @Value("${app.name:local}")
  private String appName;
}
```

Only source-ranged v0.91 keys in `application.properties`, `application-*.properties`, `bootstrap.properties`, or `bootstrap-*.properties` participate. One unique key creates an exact class-to-key `references` edge with `framework.spring-boot.properties.direct-value.literal-key.exact-key` evidence. A missing key or duplicate key across conventional files remains an explicit unresolved `references` edge with its candidate IDs and applicable configuration paths; it never chooses a profile, default, or precedence order. The raw annotation reference facts persist with the graph, so an explicit future `sync` can safely reproject them; property values never enter those facts or edge evidence.

This is not a general Spring configuration model. It excludes YAML, Kotlin, `@ConfigurationProperties`, method or parameter annotations, aliases/wildcard imports, named arguments, concatenated or escaped strings, nested placeholders, SpEL, relaxed binding, profile activation/precedence, imports, environment overrides, validation, values, and runtime behavior.

#### Micronaut Java Controller routes

v0.94 adds a first, evidence-first Micronaut route surface for direct Java classes:
...
class CatalogController {
  ...
  @Post(uri = "/refresh")
  String refresh() { return "ok"; }
}
```

The graph receives source-ranged `GET /catalog` and `POST /catalog/refresh` route symbols, each with an exact edge to its direct local method and `framework.micronaut.direct-controller.literal-method-mapping.local-method` evidence. A marker annotation means `/`; otherwise the route accepts exactly one positional string literal or one `value` literal, and method mappings also accept one `uri` literal. The class and method annotation must be proven by one precise non-static, non-wildcard import or by their fully-qualified annotation names.

This slice deliberately excludes Kotlin/Groovy, wildcard/static imports, annotation aliases or meta-annotations, `uris` arrays, media-type/port/condition arguments, multiple mapping annotations, custom HTTP methods, controller inheritance, programmatic `RouteBuilder` registrations, filters/error handlers, dependency injection, validation/OpenAPI, compilation, and runtime routing. A malformed Java file, unproven import, dynamic URI, metadata-bearing annotation, or multiple candidate HTTP mappings emits no Micronaut route.

#### Shell/Bash direct functions

v0.93 adds a dependency-free `shell` artifact language for `.sh` and `.bash` files. It retains only complete declarations that begin at the direct outer script level:
...
The graph receives a file symbol plus source-ranged `deploy` and `cleanup` `function` symbols, each with an exact `contains` edge carrying `language.shell.function.direct-top-level` evidence. String literals, comments, escapes, and `${parameter}` expansions are masked before brace matching, so text that merely resembles a function does not become a declaration. Complete direct functions are queryable through the existing source search, CLI, MCP, node, and graph persistence surfaces.

This deliberately excludes all execution semantics: command calls, `source` / dot imports, aliases, functions nested in control flow or groups, `eval`, here-documents, command substitutions, pipelines, redirections, arrays, trap handlers, `zsh` / fish dialects, extensionless shebang scripts, shellcheck parsing, module/package resolution, environment variables, and runtime behavior. Unterminated strings, escapes, parameter expansions, or direct outer-script control/brace forms—and any here-document—leave only the file symbol.

#### SQL DDL resources

v0.95 adds a dependency-free `sql` artifact language for case-insensitive `.sql` filenames. It records a schema resource only after it finds one complete, semicolon-terminated direct declaration with an unquoted literal name:
...
The graph receives `resource` symbols named `public.users` and `public.active_users`, qualified as `db/schema.sql#sql-table:public.users` and `db/schema.sql#sql-view:public.active_users`. Each has an exact file-to-resource `contains` edge with `language.sql.create-table.direct-ddl` or `language.sql.create-view.direct-ddl` evidence. Table bodies and view queries are preserved only as source context; no column, constraint, query, relation, or execution fact is inferred.

The lexical boundary masks `--` and `/* ... */` comments, quoted string data, double/backtick/bracket identifier text, and escaped quote forms before statement selection. Unterminated comments or quotes, dollar-quoted source (including routine bodies), quoted or dynamic resource names, `CREATE TABLE AS`, materialized views, view column lists, DML, `ALTER` / `DROP`, stored procedures/functions, migration ordering, dialect selection, database connectivity, and runtime behavior remain outside this release.

#### GraphQL schema declarations

v0.96 adds a dependency-free `graphql` artifact language for `.graphql`, `.gql`, and `.graphqls` files. It retains a schema definition only after a complete direct lexical boundary is proven:
...
The graph receives a `class` for object `type User`, an `interface` for `interface Node`, and existing `type` symbols for input, enum, scalar, and union definitions. They are qualified as `api/schema.graphql#type:User`, `#interface:Node`, `#input:CreateUserInput`, `#enum:Role`, `#scalar:DateTime`, and `#union:SearchResult`; each exact file containment edge carries a `language.graphql.<kind>.direct-definition` rule ID.

The scanner masks `#` comments, standard strings, and block-string descriptions while preserving offsets, then requires globally balanced braces, parentheses, and list delimiters. Directives and `implements` clauses are tolerated only as header context: fields, field arguments, value defaults, directives, type references, interface implementation edges, union-member edges, root-schema mapping, schema extensions, query/mutation/subscription field semantics, validation, code generation, and runtime service behavior are deliberately not inferred. Any malformed delimiter/string, or a top-level `extend` declaration, leaves only the file symbol.

v0.97 adds one deliberately narrow framework bridge. A direct, non-type-only named import (including an alias) must prove exactly one synchronous `@Resolver(() => User)` class decorator; the factory must have no parameters and an expression-bodied identifier. If exactly one indexed GraphQL object `type User` exists, the resolver class receives a `references` edge with `heuristic` resolution, confidence `0.85`, and `framework.nestjs.graphql.resolver-schema.unique-object-type` evidence. Missing or duplicate object types become target-null `unresolved` edges with candidate IDs instead. This does not claim that the TypeScript value and schema type are semantically identical, nor does it connect `@Query`, `@Mutation`, `@Subscription`, fields, arguments, code-first schemas, federation, transport, or runtime execution.

#### Pascal

v0.75 adds `.pas`, `.dpr`, `.dpk`, and `.lpr` discovery through a direct lexical implementation boundary:
...
This emits direct `contains` facts for `THealthService.Ping`, `THealthService.Reset`, and `Add`; the interface-only declaration emits nothing. The scanner accepts only a column-one `procedure` / `function` header with a complete outer `begin ... end;` implementation. It blanks `//`, `{...}`, `(*...*)`, and quoted strings before scanning, and tracks simple nested `begin`, `case`, `try`, `repeat`, and direct type blocks so their closing markers do not terminate the routine early.

The v0.75 declaration pass deliberately excludes `.dfm` / `.fmx`, forward/interface declarations, indented/local routines, constructors/destructors/operators/generics, overload/directive forms, unit/module/call/type/class relationships, VCL/FMX/Lazarus form semantics, compiler proof, and runtime behavior. Unterminated comments/strings or an incomplete routine retain only a file symbol.

##### Horse (Pascal)

v0.82 expands the deliberately separate Horse route pass to HEAD while preserving the same fully local shape:
...
The program creates `GET /health`, `POST /users`, `PUT /users`, `PATCH /users`, `DELETE /users`, and `HEAD /health` route symbols and exact `routes` edges to their unique, prior same-file routines, each carrying `framework.horse.direct-uses.literal-route.local-routine` syntax evidence. The source must contain exactly one standalone direct `uses Horse;`, exactly one direct `program` declaration, and one complete top-level `begin ... end.` block. Only one-line slash-prefixed literal `THorse.Get` / `THorse.Post` / `THorse.Put` / `THorse.Patch` / `THorse.Delete` / `THorse.Head` calls at that main-block level are accepted.

Horse support deliberately excludes combined or aliased `uses`, units, inline or multiline callbacks, `Options` and other HTTP methods, `THorse` aliases/subclasses/wrappers, groups/prefixes/middleware, nested or routine-local registrations, dynamic/query/fragment/double-slash paths, late/ambiguous/cross-file handlers, compilation, and runtime behavior. This is a narrow static evidence surface, not a claim of general Horse compatibility.
...
#### Twig (literal templates-root relationships)

v0.71 adds a deliberately small Twig relation surface for conventional project layouts:
...
| Accepted tag | Required form | Exact project-local target |
| --- | --- | --- |
| `extends` | a complete direct safe literal `.twig` name | `templates/<name>.twig` |
| `include` / `embed` | a complete direct literal target with no tail, `only`, or `ignore missing` | `templates/<name>.twig` |
| `import` | a complete direct literal target plus `as <identifier>` | `templates/<name>.twig` |
| `from` | a complete direct literal target plus a simple `import <identifier>` list | `templates/<name>.twig` |
...
#### Nix (returned attribute-set and import-syntax evidence)

v0.69 adds a deliberately narrow `.nix` declaration surface:
...
| Accepted source form | Evidence |
| --- | --- |
| Complete returned literal attribute set, including `rec { ... }` | Direct binding becomes an exported function or variable symbol |
| Complete direct `let ... in` binding | A non-exported function or variable symbol with a distinct lexical scope |
| Complete simple `inherit a b;` form | Direct inherited names become variable symbols |
| Literal project-relative `import ./file.nix` or `builtins.import ../file.nix` | An explicit pending `imports` reference, without a guessed target edge |

The scanner masks Nix line/block comments plus double-quoted and basic indented strings while preserving source offsets. It fails closed for unclosed or ambiguous comments, strings, delimiters, and `let` expressions. It deliberately excludes quoted/dynamic attribute names, nested attribute members, `with`, assertions, overlays, derivations, flake inputs/outputs, angle-bracket lookups, import-target resolution, `callPackage`, arbitrary calls, package dependency/evaluation semantics, NixOS/Home Manager module composition, lock files, builds, and runtime deployment behavior.

#### VB.NET (complete declaration and Imports evidence)

v0.70 adds a deliberately narrow `.vb` declaration surface:
...
| Accepted source form | Evidence |
| --- | --- |
| Complete `Namespace ... End Namespace` | A module-kind namespace symbol |
| Complete `Class` / `Module` / `Interface` / `Structure` / `Enum` block | A contained class/module/interface/type symbol |
| Complete direct `Sub ... End Sub` / `Function ... End Function` | A direct function or member method symbol |
| Direct interface or `MustOverride` callable signature | A bodyless method symbol |
| File-level unaliased `Imports Namespace.Name` | An explicit pending `imports` reference, without a CLR/project target-resolution claim |

The scanner masks apostrophe / `Rem` comments and double-quoted strings while preserving source offsets. It fails closed for unclosed strings and mismatched or incomplete supported `End ...` blocks. It deliberately excludes attributes, import aliases, fields, properties, events, delegates, P/Invoke, generic/overload/type semantics, inheritance, `Handles`, calls, lambdas/local functions, partial-type merging, project/assembly/NuGet/MSBuild resolution, compilation, designer resources, and runtime UI behavior.
...
v0.73 adds a deliberately small Django URLConf proof. It accepts a direct named import of `path` from `django.urls` (an import alias is allowed), then only the final literal top-level `urlpatterns` list and a prior same-file top-level function handler:
...
This emits `ALL / -> home` and `ALL /users/<int:user_id>/ -> user_detail` with exact `framework.django.direct-urlpatterns.path.local-function` syntax evidence. `ALL` is intentional: a Django URL pattern does not statically establish one HTTP method.

The v0.73 Django contract accepts only:

- a syntactically valid Python file with a direct one-line `from django.urls import path [as alias]` import that has not been rebound before the final list;
- exactly the final top-level `urlpatterns = [...]` assignment, whose items are direct calls to that imported local path factory;
- a plain unescaped path without a leading slash, backslash, query, fragment, or `..` segment; the empty string maps to `/` and literal converter segments are retained;
- a direct prior same-file top-level function name as the handler; and
- no route metadata except an optional literal `name="..."` keyword.

It deliberately excludes `re_path`, legacy `url`, `include`, DRF/router registrations, `as_view`, dotted/imported/cross-file handlers, tuples and `urlpatterns +=`, assignment aliases, nested route arrays, extra options such as `kwargs`, dynamic/escaped values, middleware/settings/namespaces, and runtime URL resolution.
...
v0.84 adds a separately proven Go Fiber capability for direct v2/v3 App and Group registrations. It follows the official [Fiber routing guide](https://docs.gofiber.io/guide/routing/), but retains only a strict static subset:
...
This emits `GET /health -> health` with `framework.fiber.direct-app.method.local-function` evidence and `POST /api/users -> createUser` with `framework.fiber.direct-group.method.local-function` evidence. Direct `Get`, `Post`, `Put`, `Patch`, `Delete`, `Head`, `Options`, `Trace`, `Connect`, and `All` are supported; `All` is represented as `ALL`. A direct `Get` is retained as source registration rather than synthesizing Fiber's automatic `HEAD` behavior.

The v0.84 Fiber contract accepts only:

- a syntactically valid `.go` file and one direct non-dot/non-blank import of `github.com/gofiber/fiber/v2` or `github.com/gofiber/fiber/v3`, using the default `fiber` name or a direct alias;
- a same-function short-variable App binding exactly like `app := fiber.New()`, whose package alias is not shadowed before the binding;
- direct `group := receiver.Group("/prefix")` bindings with one literal slash-prefixed, non-root, non-trailing, non-duplicated-slash prefix; nested proven groups compose their literal prefixes;
- a direct App/Group HTTP method with one literal slash-prefixed path and exactly one named package-level function handler; and
- no local shadowing of the handler name before the proven registration.

It intentionally excludes `fiber.New(fiber.Config{...})`, `Use`, `Route`, `RouteChain`, mounted sub-apps, group middleware, inline/multiple/wrapped handlers, dynamic or escaped paths, root/trailing/double-slash group prefixes, chained/member receivers, factory/wrapper construction, cross-file receiver flow, Go module/package resolution, methods, automatic `HEAD`, and runtime routing behavior.

#### Echo (Go)

v0.86 adds a separately proven Go Echo capability for direct v4/v5 App and Group registrations. It follows the official [Echo routing guide](https://echo.labstack.com/docs/routing), but retains only a strict static subset:
...
This emits `GET /health -> health` with `framework.echo.direct-app.method.local-function` evidence and `ALL /api/fallback -> fallback` with `framework.echo.direct-group.method.local-function` evidence. Direct `GET`, `POST`, `PUT`, `PATCH`, `DELETE`, `HEAD`, `OPTIONS`, and `Any` are supported; `Any` is represented as `ALL`.

The v0.86 Echo contract accepts only:

- a syntactically valid `.go` file and one direct non-dot/non-blank import of `github.com/labstack/echo/v4` or `github.com/labstack/echo/v5`, using the default `echo` name or a direct alias;
- a same-function short-variable App binding exactly like `app := echo.New()`, whose package alias is not shadowed before the binding;
- direct `group := receiver.Group("/prefix")` bindings with one literal slash-prefixed, non-root, non-trailing, non-duplicated-slash prefix; nested proven groups compose their literal prefixes;
- a direct App/Group HTTP method with one literal slash-prefixed path and exactly one named package-level function handler; and
- no local shadowing of the handler name before the proven registration.

It intentionally excludes route or group middleware, `Match`, `File` / static helpers, inline/multiple/wrapped handlers, dynamic or escaped paths, parameter/wildcard semantics, root/trailing/double-slash group prefixes, chained/member receivers, factory/wrapper construction, cross-file receiver flow, Go module/package resolution, methods, compiler validation, and runtime routing behavior.
...
Adjacent exact references are also checked as a directed static path in the supplied order. A returned `path` is the deterministic shortest path through **exact** `calls`, `references`, `routes`, `handles`, or `imports` edges only. SymbolLattice never reverses an edge, treats a heuristic edge as proof, or fabricates a dynamic-dispatch hop. Each pair reports one of `path`, `same-symbol`, `no-path`, `not-applicable`, or `truncated`.
...
| `routes [path]` | List bounded static Express, Fastify, Flask, FastAPI, Django, Gin, Fiber, Echo, `net/http`, Chi, and Axum route nodes (including direct inline, same-file named, and imported/re-exported Fastify plugin-prefix projections) plus AST-proven NestJS route nodes with exact `RouterModule.register()` prefix projections; Django URLConf entries use `ALL` because URL patterns do not prove one HTTP method; accepts `--method` (including `TRACE` and `CONNECT`), `--path`, and `--limit`; never refreshes the index |
...
v0.69 adds no SQLite schema migration or query command. It adds Nix `.nix` discovery plus complete direct returned-attribute-set, `let ... in`, and simple `inherit` declaration facts through the existing file, symbol, containment-edge, source-search, pending-reference, and raw-artifact contracts. Literal project-relative `import` syntax is retained without a module target-resolution claim. The extractor advances to `multi-language-ast-v58`; the resolver remains `project-resolver-v21` because v0.69 does not evaluate Nix expressions or project cross-file Nix module edges. A pre-v0.69 active index reports `indexer-version-changed` until an explicit `sync` or `index` republishes Nix-capable facts. Existing generations remain readable.

v0.70 adds no SQLite schema migration or query command. It adds VB.NET `.vb` discovery plus complete namespace, primary type, and direct callable facts through the existing file, module/class/interface/type/function/method, containment-edge, source-search, pending-reference, and raw-artifact contracts. Simple file-level unaliased `Imports` syntax is retained without an assembly/project target-resolution claim. The extractor advances to `multi-language-ast-v59`; the resolver remains `project-resolver-v21` because v0.70 does not resolve CLR assemblies, projects, packages, or source relationships. A pre-v0.70 active index reports `indexer-version-changed` until an explicit `sync` or `index` republishes VB.NET-capable facts. Existing generations remain readable.
...
v0.73 adds no SQLite schema migration or query command. It adds the `django` framework capability and exact same-file Django `ALL` route evidence only for direct `django.urls.path` import aliases, a final literal top-level `urlpatterns` list, and a prior unique local function handler. The artifact extractor advances to `multi-language-ast-v62`; the resolver remains `project-resolver-v23` because this initial Django slice has no cross-file projection. A pre-v0.73 active index reports `indexer-version-changed` until an explicit `sync` or `index` republishes Django-capable facts. Existing generations remain readable.

v0.74 adds no SQLite schema migration or query command. It adds `.luau` discovery plus direct balanced Lua-compatible top-level function containment, including `export function` under `--!strict` and simple type-annotation context. The artifact extractor advances to `multi-language-ast-v63`; the resolver remains `project-resolver-v23`, because Luau exposes no generic module, type, call, Roblox, or framework projection and never reuses the Lua-only Lapis route pass. A pre-v0.74 active index reports `indexer-version-changed` until an explicit `sync` or `index` republishes Luau-capable facts. Existing generations remain readable.
...
v0.79 adds direct ordinary Objective-C interfaces and protocols in .m/.mm files, one-line declaration methods, and same-file interface/implementation class merging with implementation-preferred matching selectors. The artifact extractor advances to multi-language-ast-v68; the resolver remains project-resolver-v23 because this declaration slice remains file-local. A pre-v0.79 active index reports indexer-version-changed until an explicit sync or index republishes the expanded Objective-C facts. Existing generations remain readable.
...
v0.84 adds an additive Go Fiber v2/v3 framework capability: a direct import, same-function `app := fiber.New()`, literal direct App methods, nested same-function literal Group prefixes, and a unique unshadowed same-file named package-level handler produce exact persisted route evidence. The artifact extractor advances to multi-language-ast-v73; the resolver remains project-resolver-v23 because this Fiber proof is file-local. A pre-v0.84 active index reports indexer-version-changed until an explicit sync or index republishes Fiber-capable facts. Existing generations remain readable.

v0.85 adds `.zig` discovery and direct source-proven top-level named container/function containment. The artifact extractor advances to multi-language-ast-v74; the resolver remains project-resolver-v23 because this Zig declaration proof is file-local. A pre-v0.85 active index reports indexer-version-changed until an explicit sync or index republishes Zig-capable facts. Existing generations remain readable.

v0.86 adds an additive Go Echo v4/v5 framework capability: a direct import, same-function `app := echo.New()`, literal direct App methods, nested same-function literal Group prefixes, and a unique unshadowed same-file named package-level handler produce exact persisted route evidence. The artifact extractor advances to multi-language-ast-v75; the resolver remains project-resolver-v23 because this Echo proof is file-local. A pre-v0.86 active index reports indexer-version-changed until an explicit sync or index republishes Echo-capable facts. Existing generations remain readable.

v0.87 adds `.yaml` and `.yml` discovery plus parser-proven single-document top-level scalar mapping-key containment. The artifact extractor advances to `multi-language-ast-v76`; the resolver remains `project-resolver-v23` because this YAML declaration proof is file-local and creates no cross-file configuration relation. A pre-v0.87 active index reports `indexer-version-changed` until an explicit `sync` or `index` republishes YAML-capable facts. Existing generations remain readable.
...
v0.92 adds a persisted Spring Boot direct-field `@Value` raw-fact capability and projects exact or explicit unresolved class-to-conventional-properties `references` edges only after the complete artifact catalog is known. The artifact extractor advances to `multi-language-ast-v81` and the resolver to `project-resolver-v24`, so a pre-v0.92 active index reports `indexer-version-changed` until an explicit `sync` or `index` republishes the complete property-reference projection. No SQLite schema migration or new query command is required; existing generations remain readable.

v0.93 adds `.sh` / `.bash` discovery and complete direct top-level Shell/Bash function containment. The artifact extractor advances to `multi-language-ast-v82`; the resolver remains `project-resolver-v24` because this initial language slice is file-local. A pre-v0.93 active index reports `indexer-version-changed` until an explicit `sync` or `index` republishes Shell-capable facts. No SQLite schema migration or query command is required; existing generations remain readable.

v0.94 adds direct Java Micronaut `@Controller` routes with a precise annotation import or fully-qualified-name proof, one literal class base URI, one literal method URI, and an exact local method target. The artifact extractor advances to `multi-language-ast-v83`; the resolver remains `project-resolver-v24` because all Micronaut route facts are file-local. A pre-v0.94 active index reports `indexer-version-changed` until an explicit `sync` or `index` republishes Micronaut-capable facts. No SQLite schema migration or query command is required; existing generations remain readable.

v0.95 adds `.sql` discovery plus source-ranged direct `CREATE TABLE` and `CREATE VIEW` schema-resource containment. The artifact extractor advances to `multi-language-ast-v84`; the resolver remains `project-resolver-v24` because this first SQL slice emits only file-local declaration facts. A pre-v0.95 active index reports `indexer-version-changed` until an explicit `sync` or `index` republishes SQL-capable facts. No SQLite schema migration or new query command is required; existing generations remain readable.

v0.96 adds `.graphql` / `.gql` / `.graphqls` discovery plus direct GraphQL schema-definition containment. The artifact extractor advances to `multi-language-ast-v85`; the resolver remains `project-resolver-v24` because this first GraphQL slice emits only file-local declaration facts. A pre-v0.96 active index reports `indexer-version-changed` until an explicit `sync` or `index` republishes GraphQL-capable facts. No SQLite schema migration or new query command is required; existing generations remain readable.

v0.97 retains a bounded direct NestJS `@Resolver(() => Identifier)` raw fact and projects it only to one unique indexed GraphQL object type as an explicitly heuristic cross-language `references` edge. The artifact extractor advances to `multi-language-ast-v86` and the resolver to `project-resolver-v25`; a pre-v0.97 active index reports `indexer-version-changed` until an explicit `sync` or `index` republishes the new payload and projection. No SQLite schema migration or new query command is required because `nestGraphqlFacts` is additive in the existing artifact-facts JSON; older generations remain readable without the new relation.

v0.98 adds case-insensitive `.proto` discovery plus source-ranged direct top-level Protocol Buffers `message`, `enum`, and `service` containment, with direct semicolon-terminated service RPC members. The artifact extractor advances to `multi-language-ast-v87`; the resolver remains `project-resolver-v25` because this declaration slice is file-local. A pre-v0.98 active index reports `indexer-version-changed` until an explicit `sync` or `index` republishes Proto-capable facts. No SQLite schema migration or new query command is required; existing generations remain readable.

v0.99 adds a `jakarta-rest` Java framework capability: a direct current `jakarta.ws.rs` or legacy `javax.ws.rs` `@Path` class plus exactly one direct marker HTTP annotation and optional literal method `@Path` becomes an exact local route. The artifact extractor advances to `multi-language-ast-v88`; the resolver remains `project-resolver-v25` because these route facts are file-local. A pre-v0.99 active index reports `indexer-version-changed` until an explicit `sync` or `index` republishes Jakarta-REST-capable facts. No SQLite schema migration or new query command is required; existing generations remain readable.
...
flowchart LR
  ...
  Extractor["TypeScript AST extractor + ArkTS/ArkUI lexical scanner + Vue/Svelte/Astro SFC scanners + Razor/Blazor directive scanner + Terraform/OpenTofu HCL lexical scanner + Shopify Liquid and Twig tag scanners + YAML document parser + XML event parser + Java properties key, Shell/Bash function, Protocol Buffers declaration, and GraphQL schema scanners + Solidity, CFML/CFScript, Nix, Zig, and VB.NET declaration scanners + Python/Go/Rust/Java/PHP/C/C++ Lezer parsers + Lua/Luau/Pascal/R/Elixir/Erlang/Clojure/Perl/Julia/Haskell lexical extractors + C#/Ruby/Kotlin/Swift/Dart/Scala ast-grep parsers\nfirst-party framework capability passes\nExpress/Fastify/Nest HTTP routes + FastAPI/Flask/Django/Gin/Fiber/Echo/net-http/Chi/Axum/Spring Web/Micronaut/Jakarta REST/Spring Boot properties/Laravel/Drupal routing YAML/CivetWeb/Lapis/Horse/Plumber/Phoenix/Cowboy/Compojure/Dancer2/Genie/Scotty/cpp-httplib/ASP.NET Core/Rails/Ktor/Vapor/Flutter/Play facts\nFlutter/Vue Router/SvelteKit/Astro/Blazor/React Router/Next client navigation\nNest module-prefix facts + non-HTTP and ArkUI UI-root entrypoints + Terraform/OpenTofu IaC declaration facts + Shopify Liquid and Twig local template calls + Solidity hierarchy + CFML/CFScript, Nix, Zig, VB.NET, YAML, XML/MyBatis, Java properties, Shell/Bash, Protocol Buffers, and Spring Boot direct-field reference facts"] --> Facts["Reusable artifact facts"]
  Micronaut["Java Micronaut direct Controller\nliteral local route pass"] --> Facts
...
src/
  ...
  extraction/      TypeScript AST, ArkTS/ArkUI lexical, Vue/Svelte/Astro SFC, Razor/Blazor directive, Terraform/OpenTofu HCL lexical, Shopify Liquid/Twig tag lexical, YAML document plus Drupal routing YAML, XML/MyBatis event parsing, Java Spring/Micronaut/Jakarta REST routes plus properties key, Shell/Bash direct function, SQL DDL, GraphQL schema, Protocol Buffers declarations, plus direct-field Spring Boot @Value parsing, Solidity declaration lexical, CFML/CFScript declaration lexical, Nix declaration lexical, VB.NET/COBOL/Zig declaration lexical, Python/Go/Rust/Java/PHP/C/C++ Lezer, Lua/Luau/Pascal/Horse/Objective-C/R/Elixir/Erlang/Clojure/Perl/Julia/Haskell lexical, and C#/Ruby/Kotlin/Swift/Dart/Scala ast-grep fact extraction
...
- The Java properties surface proves only physically contiguous, non-empty literal keys. It accepts `=`, `:`, whitespace-separated, and no-value forms and decodes escaped separators, whitespace, and `\\uXXXX` sequences only in the key identity. The separate Spring Boot proof accepts only direct Java class fields with a precise or fully-qualified `@Value` annotation containing one plain `${key}` or `${key:default}` literal, and only tries literal keys in conventional `application` / `bootstrap` `.properties` files. It excludes key continuations, malformed escapes, control-character keys, values and value semantics, Spring profile precedence/placeholders/defaults at runtime, aliases/wildcard imports, named/dynamic/nested/SpEL `@Value` arguments, `@ConfigurationProperties`, YAML/Kotlin configuration, framework auto-detection, broad cross-file configuration resolution, schema validation, encoding/runtime loading behavior, and runtime analysis. A dangling continuation or unsupported key form produces no property symbol; its source file remains indexed.

- The Micronaut Java surface proves only direct `io.micronaut.http.annotation.Controller` and one of `Get`, `Post`, `Put`, `Patch`, `Delete`, `Head`, `Options`, or `Trace` through one exact import or fully-qualified annotation. It accepts a marker `/`, one positional URI literal, one named `value`, or a method-only named `uri`, then joins the direct class and method paths to one exact local handler. It excludes Kotlin/Groovy, static/wildcard imports, composed annotations, `uris` arrays, media/port/condition arguments, multiple mappings, custom verbs, dynamic strings, controller inheritance, programmatic RouteBuilder routes, filters/error handlers, injection/validation/OpenAPI, compilation, and runtime behavior.

- The Jakarta REST / JAX-RS Java surface proves only one direct imported or fully-qualified `jakarta.ws.rs.Path` or `javax.ws.rs.Path` class annotation with one plain string value, one direct marker `GET` / `POST` / `PUT` / `PATCH` / `DELETE` / `HEAD` / `OPTIONS` annotation, and optional one direct literal method `@Path`. It accepts relative and slash-prefixed template text only as a static path string, then joins it to the direct local method. It excludes `ApplicationPath`, sub-resource locators, custom `@HttpMethod` annotations, method `@Path` without a request method, multiple/dynamic/escaped paths, request/response parameter binding, `Consumes` / `Produces`, media negotiation, exception mappers, filters/interceptors, CDI/RESTEasy/Quarkus configuration, inherited/cross-file handlers, OpenAPI, compilation, and runtime behavior.

- The Shell/Bash surface proves only complete column-one direct POSIX `name() { ... }` and Bash `function name { ... }` declarations in `.sh` / `.bash` files. It masks strings, comments, escapes, and parameter expansions for source-range-safe brace matching, and it rejects here-documents plus malformed top-level quote/escape/parameter/control/brace forms. It excludes extensionless shebang files, zsh/fish dialects, command/call facts, `source`/dot imports, aliases, functions nested in control flow or groups, `eval`, here-documents, command substitution/pipeline/redirection/array/trap semantics, module resolution, environment variables, lint/compiler integration, and runtime behavior.
- The SQL surface proves only semicolon-terminated direct unquoted `CREATE TABLE` and direct `CREATE [OR REPLACE] [TEMP] VIEW` statements with a simple qualified ASCII identifier. It emits a file-contained schema `resource` only; table bodies and view queries are never interpreted. It excludes quoted/dynamic identifiers, `CREATE TABLE AS`, table options after the closing body, materialized/recursive/column-list views, DML/DDL beyond this subset, columns/constraints/indexes/types/sequences/triggers, table/view dependencies, SQL routines/procedures/functions, migration ordering, dialect selection, database connectivity, execution plans, validation, and runtime behavior. Unterminated comments or quotes and dollar-quoted source retain only the file symbol.
- The GraphQL surface proves only direct braced `type`, `interface`, `input`, and `enum` definitions plus direct simple `scalar` and `union` definitions. v0.97 additionally retains one direct NestJS `@Resolver(() => Identifier)` reference and projects it only to a unique indexed object `type` as heuristic evidence; target absence or ambiguity is explicit unresolved evidence. It excludes type extensions, field/argument/default-value facts, directives and descriptions as semantic facts, implements/union-member relations, root schema mapping, schema federation, fragments and operation documents, resolver field linkage, introspection, validation, code generation, query planning, code-first schemas, transport, server connection, and runtime behavior. An unclosed string, mismatched delimiter, unsupported token, or top-level `extend` declaration retains only the file symbol.
- The Protocol Buffers surface proves only balanced direct top-level `message`, `enum`, and `service` blocks, plus direct semicolon-terminated `rpc Name(Request) returns (Response);` service members with optional `stream` and dotted message names. It masks comments and quoted values without retaining them, excludes nested declarations, fields, maps, oneofs, enum values, imports, packages, syntax/edition validation, options, reserved/extensions, RPC option blocks, request/response type references, HTTP annotations, generated code, gRPC transport, cross-file contract resolution, compiler validation, and runtime behavior. Unterminated comments/strings, unbalanced delimiters, or unsupported tokens retain only the file symbol.
...
- The Python/FastAPI surface proves one narrow cross-file router form: direct import aliases, direct `APIRouter` construction, literal router/include prefixes, same-file composition, and a one-dot regular-package direct router import are supported. Flask supports direct app and same-file Blueprint registrations with literal methods/prefixes. Django supports only direct `path` imports, a final literal same-file `urlpatterns` list, and prior direct local function handlers; it does not model `re_path`, `include`, DRF, class-based views, or cross-file URLConf/view resolution. None of these packs resolves generic Python imports/exports/calls, parent-relative or namespace-package FastAPI imports, cross-file Flask Blueprints, member routers, import/re-export chains, nested routers, assignment aliases, dependencies or middleware as graph relationships, factory composition, star/keyword expansion, possible rebindings, dynamic/escaped paths or prefixes, or routers declared after inclusion. Syntax-error Python files retain only their file symbol until repaired.
- The Go surface proves top-level functions, direct same-function `:= gin.Default()` / `gin.New()` receivers, direct v2/v3 `app := fiber.New()` receivers, direct v4/v5 `app := echo.New()` receivers, literal one-handler Gin, Fiber, and Echo App registrations with literal non-root/non-trailing Group prefixes, direct `http.HandleFunc`, same-function `mux := http.NewServeMux()` / `mux.HandleFunc` registrations, and direct `chi.NewRouter()` / `chi.NewMux()` receiver methods. It excludes `var` bindings, framework-specific constructor configuration, `Use` / `Route` / `RouteChain` / mounted Fiber sub-apps, `Handle` / `Match`, static helpers, inline/multiple/middleware or wrapped handlers, path escaping/dynamic values, root/trailing/double-slash Gin/Fiber/Echo group prefixes, Chi `Route` / `Group` / `Mount` composition, factory/wrapper/chained/mutable receiver flow, `DefaultServeMux` member calls, automatic Fiber `HEAD`, host/wildcard pattern semantics, cross-file module/package resolution, methods, generic Go imports/calls/types, and runtime framework behavior. Syntax-error Go files retain only their file symbol until repaired.
...
- The Luau surface reuses only the balanced Lua-compatible declaration subset for `.luau`: direct top-level `function`, `local function`, and `export function`. `--!strict`, type aliases, and annotations are tolerated but never become type facts; no Lua Lapis route pass, `require`/module/call resolution, generic functions, Roblox service/`Instance`/event/`RemoteEvent` knowledge, Roact/Fusion framework inference, table/metatable semantics, compiler proof, or runtime behavior is claimed. Unbalanced input retains only its file symbol until repaired.
- The Pascal surface is a direct lexical implementation subset, not a Pascal grammar, compiler, or project model. It accepts only column-one complete `procedure` / `function` implementations, including simple dotted and `class` names, after blanking `//`, `{...}`, `(*...*)`, and quoted strings. It tracks simple nested `begin`, `case`, `try`, `repeat`, and direct type blocks solely to locate a complete outer `end;`. Its only framework exception is Horse: exactly one `uses Horse;`, one `program` main block, one-line literal top-level `THorse.Get` / `THorse.Post` / `THorse.Put` / `THorse.Patch` / `THorse.Delete` / `THorse.Head`, and a unique prior same-file complete routine handler. It excludes `.dfm` / `.fmx`, forward/interface/indented/local routines, constructors/destructors/operators/generics, overload/directive forms, type/class/interface/unit/general-uses/call relationships, forms, all other Horse/Brook/WebBroker/framework forms, compilation, and runtime behavior. Unterminated comments/strings or incomplete routines retain only the file symbol until repaired.
...
| `v0.73.0` | Django direct `path` import aliases, final literal same-file `urlpatterns`, exact prior local-function `ALL` routes, persisted extractor provenance, and dynamic/rebound/late/nonlocal/`include`/`re_path`/class-based/extra-option rejection |
| `v0.74.0` | Luau `.luau` discovery, direct Lua-compatible top-level function containment, strict/type-annotation tolerance, source-search/CLI/MCP filtering, explicit Lua-Lapis isolation, and malformed-source fail-closed behavior |
| `v0.75.0` | Pascal `.pas` / `.dpr` / `.dpk` / `.lpr` discovery, direct complete `procedure` / `function` containment, dotted/class routine support, comments/strings/nested-block isolation, source-search/CLI/MCP filtering, and malformed-source fail-closed behavior |
| `v0.76.0` | Pascal Horse exact local `GET` / `POST` route facts only after one direct `uses Horse;`, one direct program main block, one-line literal registration, and a unique prior same-file complete routine handler |
| `v0.77.0` | Objective-C `.m` / Objective-C++ `.mm` discovery, direct complete non-category implementation containment, one-line brace-bodied selector methods, source-search/CLI/MCP filtering, and malformed-source fail-closed behavior |
...
| `v0.84.0` | Go Fiber v2/v3 direct import and `app := fiber.New()` proof, literal App/Group methods with nested same-function prefix composition, unique same-file package-level handlers, persisted route querying, and dynamic/shadow/configured/middleware/rebinding rejection |
| `v0.85.0` | Zig `.zig` discovery, direct balanced top-level named `struct` / `enum` / `union` / `opaque` containers and named `fn` declarations, public-visibility retention, persisted source search, and comment/string/nested/malformed rejection |
...
| `v0.92.0` | Conservative Spring Boot direct Java field `@Value` facts, persisted exact/unresolved class-to-conventional-properties `references` evidence, duplicate/missing candidate disclosure without profile-precedence guessing, and static reference traversal support |
| `v0.93.0` | Shell/Bash `.sh` / `.bash` discovery, complete direct top-level POSIX/Bash function containment with source ranges, persisted source search, and nested/incomplete/quoted/here-document rejection |
| `v0.94.0` | Import-proven direct Java Micronaut `@Controller` plus literal `Get` / `Post` / `Put` / `Patch` / `Delete` / `Head` / `Options` / `Trace` mappings, source-ranged exact local route edges, source search, and dynamic/wildcard/multiple-mapping rejection |
| `v0.95.0` | SQL `.sql` discovery, complete direct unquoted `CREATE TABLE` / `CREATE VIEW` schema-resource containment with source ranges and persisted source search, plus comment/string/quoted-name/dollar-routine rejection |
| `v0.96.0` | GraphQL `.graphql` / `.gql` / `.graphqls` discovery, direct object/interface/input/enum/scalar/simple-union schema declarations with source ranges and persisted source search, plus description/string/operation/extension rejection |
| `v0.97.0` | Bounded direct NestJS `@Resolver(() => Identifier)` raw facts, SQLite persistence, and resolver-class-to-unique-GraphQL-object-type heuristic `references` edges with explicit missing/ambiguous candidate evidence |
| `v0.98.0` | Protocol Buffers `.proto` discovery, direct top-level `message` / `enum` / `service` declarations and semicolon RPC member containment with source ranges and persisted source search, plus nested/malformed/RPC-block rejection |
| `v0.99.0` | Java Jakarta REST / JAX-RS direct current/legacy `@Path` resources, literal optional method paths, exact local HTTP request-method route edges, source search, and dynamic/wildcard/multiple-method rejection |
...
| `v0.92+` | Java properties key continuations, profile/precedence/default/placeholder/interpolation semantics, value analysis, broader `@Value` / `@ConfigurationProperties` binding, framework detection, YAML/Kotlin and broad cross-file configuration resolution, schema validation, encoding/runtime loading behavior, and runtime analysis |
| `v0.93+` | Extensionless shebang scripts, zsh/fish dialects, a real Shell grammar/shellcheck validation, command/source/call/import relationships, aliases/eval/functions in groups or control flow, here-documents, command substitution/pipelines/redirections/arrays/traps, module/package/environment analysis, and runtime behavior |
| `v0.94+` | Kotlin/Groovy Micronaut, wildcard/static imports, annotation aliases/meta-annotations, `uris` arrays, media/port/condition argument interpretation, multiple/custom mappings, dynamic URI values, inheritance, RouteBuilder/programmatic routes, filters/error handlers, injection/validation/OpenAPI, compilation, and runtime routing |
| `v0.95+` | SQL dialect/parser validation, quoted/dynamic names, `CREATE TABLE AS`, materialized/recursive/column-list views, tablespace/option clauses, columns/constraints/indexes/types/sequences/triggers, DML, routines/procedures/functions, migration ordering, cross-file schema relations, query analysis, database connectivity, compiler validation, and runtime behavior |
| `v0.96+` | GraphQL AST/schema validation, extensions/federation, fields/arguments/defaults/directives/descriptions, implements and union-member relations, root schema mapping, imports, cross-file schema composition, operations/fragments/introspection, code generation, transport/server analysis, and runtime behavior |
| `v0.97+` | NestJS resolver fields and arguments, `@Query` / `@Mutation` / `@Subscription` field linkage, code-first schema proof, decorator barrels/wrappers/namespace forms, semantic type compatibility, federation, multi-schema ownership, transport wiring, and runtime execution |
| `v0.98+` | Proto syntax/edition validation, packages/imports, fields/maps/oneofs/options/enum values, nested declarations, RPC option blocks, request/response type references, gRPC/HTTP transport bindings, descriptor/code generation, cross-file contract resolution, compiler validation, and runtime behavior |
| `v0.99+` | Jakarta REST `ApplicationPath`, resource-locator and inherited routes, custom HTTP methods, URI-template/parameter validation, media/content negotiation, request/response binding, CDI/RESTEasy/Quarkus configuration, OpenAPI, cross-file handlers, compiler validation, deployment, and runtime routing |
...
Python coverage includes `.py` discovery, direct declaration/containment extraction, malformed-source fail-closed behavior, direct FastAPI alias/application/decorator evidence, same-file plus one-dot regular-package cross-file `APIRouter` literal-prefix/`include_router` composition, direct Flask app and same-file `Blueprint` literal-prefix routes, direct Django `path` import alias plus final literal `urlpatterns` local-function `ALL` routes, persisted fact/evidence boundaries, source search, CLI/MCP language filters, and incremental indexing.

Go coverage includes `.go` discovery, persisted source search and CLI/MCP language filters, conservative top-level function containment, direct/default-or-aliased Gin engine creation, direct/default-or-aliased Fiber v2/v3 `App` creation, direct/default-or-aliased Echo v4/v5 `App` creation, direct literal Gin/Fiber/Echo App and nested same-function `RouterGroup` / `Group` routes, direct/default-or-aliased `net/http` `HandleFunc` plus same-function `ServeMux` routes, direct/default-or-aliased Chi `NewRouter` / `NewMux` routes, Go 1.22 literal method-pattern and `CONNECT` evidence, dynamic/shadow/configured/middleware/rebinding rejection, malformed-source fail-closed behavior, and exact route-query integration.
...
Java properties coverage includes case-insensitive `.properties` discovery, persisted source search and CLI/MCP language filters, literal non-empty key variables across `=`, `:`, whitespace-separated, and no-value forms, escaped separator/whitespace/`\\uXXXX` key decoding, exact key-only containment evidence, duplicate ordinals, value-continuation safety, and malformed/continued/dangling-key exclusion. The separate v0.92 Spring Boot slice can use only conventional application/bootstrap properties keys for exact or explicit unresolved direct Java-field `@Value` references; it makes no value, profile-precedence, placeholder-runtime, broad Spring-binding, YAML, Kotlin, or runtime claim.

Shell/Bash coverage includes `.sh` / `.bash` discovery, persisted source search and CLI/MCP language filters, complete direct top-level POSIX and Bash function containment, source ranges, quoted/commented/parameter-expansion masking, nested-control/group rejection, malformed-input fail-closed behavior, and here-document rejection. It emits no command, call, source/import, environment, module, lint, or runtime claim.

SQL coverage includes case-insensitive `.sql` discovery, persisted source search and CLI/MCP language filters, source-ranged exact containment for direct unquoted semicolon-terminated `CREATE TABLE` and `CREATE VIEW` declarations, comment/string/quoted-identifier masking, and incomplete/dollar-quoted-routine rejection. It emits no column, constraint, query, dependency, migration, database, validation, or runtime claim.

GraphQL coverage includes case-insensitive `.graphql` / `.gql` / `.graphqls` discovery, persisted source search and CLI/MCP language filters, source-ranged exact containment for direct object/interface/input/enum/scalar/simple-union schema definitions, comment/string/block-description masking, and unbalanced/extension/operation-only rejection. It also covers persisted direct NestJS `@Resolver(() => Identifier)` facts plus one-unique-object-type heuristic resolver `references` evidence, including explicit ambiguous candidates. It emits no field, directive, resolver-field, schema-composition, operation, validation, server, or runtime claim.

Jakarta REST coverage includes direct current `jakarta.ws.rs` and legacy `javax.ws.rs` `Path` import/fully-qualified proof, one literal class path, optional one literal method path, exact direct local marker HTTP-method routes, source ranges, capability-registry proof, persisted facts, source search, and route queries. It rejects wildcard imports, dynamic or duplicate paths, multiple/unproven HTTP annotations, invalid named `Path` forms, `ApplicationPath`, parameter binding, configuration, and runtime claims.

Protocol Buffers coverage includes case-insensitive `.proto` discovery, persisted source search and CLI/MCP language filters, source-ranged exact containment for direct top-level `message`, `enum`, and `service` declarations, and direct semicolon-terminated RPC methods with optional `stream` and dotted message names. It proves nested-declaration exclusion, comments/string masking, unbalanced-input fail-closed behavior, RPC-block exclusion, persisted raw facts, and service-symbol retrieval; it emits no field, option, import, request/response reference, gRPC transport, compiler, or runtime claim.
...
Java coverage includes `.java` discovery, persisted source search and CLI/MCP language filters, direct top-level class/method containment, direct imported or fully-qualified Spring Web controller evidence, literal class prefix plus literal method mapping extraction, direct local-method route edges, plus direct-field static `@Value` facts with exact import/fully-qualified proof for conservative conventional-properties reference projection. Wildcard/dynamic/multi-value/method-level-`RequestMapping`, alias/wildcard `@Value` imports, non-field annotations, named/dynamic/nested `@Value` arguments, and malformed-source paths are rejected; route and reference integration remain exact or explicitly unresolved.

Micronaut coverage adds direct Java `@Controller` and direct `@Get` / `@Post` / `@Put` / `@Patch` / `@Delete` / `@Head` / `@Options` / `@Trace` exact-import or fully-qualified proof, marker/default and one-literal URI handling, exact local route edges, source ranges, persisted source search, and wildcard/dynamic/metadata/multiple-mapping rejection. It does not claim Kotlin/Groovy, `uris`, custom annotations/verbs, programmatic route construction, or runtime routing.
...
Luau coverage includes `.luau` discovery, persisted source search and CLI/MCP language filters, direct Lua-compatible top-level `function` / `local function` / `export function` containment, `--!strict` plus type alias/annotation tolerance, and malformed-source fail-closed behavior. Luau deliberately emits no Lapis, generic framework, route, module, call, type, Roblox, Roact, or Fusion evidence in this release.

Pascal coverage includes `.pas`, `.dpr`, `.dpk`, and `.lpr` discovery, persisted source search and CLI/MCP language filters, direct column-one complete `procedure` / `function` containment, direct dotted/class routine names, `//` / brace / paren-star comment and quoted-string isolation, simple nested block handling, interface/declaration/indented/incomplete-routine rejection, malformed comment/string fail-closed behavior, plus direct Horse `GET` / `POST` / `PUT` / `PATCH` / `DELETE` route evidence only after exact `uses Horse;`, program-main-block, literal registration, and unique prior local-routine proof.
...
Nix coverage includes `.nix` discovery, persisted source search and CLI language filters, complete direct returned-attribute-set and `let ... in` function/value symbols, simple `inherit` variables, literal project-relative pending import references, and comment/string/delimiter/malformed rejection.

VB.NET coverage includes `.vb` discovery, persisted source search and CLI language filters, complete namespace/class/module/interface/structure/enum and direct `Sub`/`Function` containment, bodyless interface/`MustOverride` signatures, simple pending `Imports` references, and comment/string/incomplete-or-mismatched-`End` rejection.
