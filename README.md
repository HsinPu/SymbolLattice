...
> [!IMPORTANT]
> v0.298.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 可沿著直接的 `Factory.create().execute()` 靜態 factory chain，以已證明的 top-level 回傳型別解析兩段 exact `calls`。目前只接受明確 import、完整限定名稱或唯一同 package receiver，以及唯一 static factory 與唯一直接宣告的目標方法；overload、instance factory、shadowed receiver、wildcard／未匯入型別及繼承方法都保持不解析。
