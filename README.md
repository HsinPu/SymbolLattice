...
> [!IMPORTANT]
> v0.304.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 可沿直接的 `Factory.create().execute()` 靜態 factory chain，以已證明的 top-level 回傳型別解析兩段 exact `calls`。method set 會合併 direct 與 inherited 的不同 signature overload；相同 signature 依 direct owner、class-over-interface 與唯一 most-specific owner 排序，interface static method 不會被當成 instance member。跨檔案 chain 只接納 public factory／target method（包含 interface implicit public）；fixed／varargs、primitive／reference widening 與 parameter specificity 共同選擇 overload。hierarchy traversal 固定限制為 16 hops、256 types，超限或相關 signature owner 歧義即保持不解析；`callType` 與 `callDispatch` receipt 揭露 canonical signature、owner kind、選擇原因、bounds 與逐段 edge。未知變數、`null`、boxing/unboxing、reference cast、泛型／array、instance factory、shadowed receiver、protected／package／private caller-context access 及完整 override-equivalent erasure 仍不會被猜成精確 edge。
