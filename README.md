...
> [!IMPORTANT]
> v0.343.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 否定 pattern guard 支援 `if (!(value instanceof DirectType name))`。v14–v23 處理可證明的 `return`／`throw`、negated `else`、無標籤與具名 loop exit、傳統 `switch break`，以及 switch-expression arrow-rule block 的 `yield`。v24／v25 再允許 abrupt statement 被一層無 catch 的 `try/finally` 包裝：try body 必須只含一個已支援 abrupt statement，finally 最多 8 個 expression、local declaration 或 empty statement，且不得含 `return`／`throw`／`break`／`continue`／`yield`。receipt 保存 wrapper、try/finally body、每個 finally statement、界限、target 與精確 scope。catch、try-with-resources、finally 控制轉移、超過界限、條件式 exit、不安全巢狀 guard、同名既有 binding、泛型與作用域外都 fail closed。
...
- `benchmark:comparison` 會在隔離的 Git 工作樹副本上，以同一組真值與查詢比較 SymbolLattice 與指定 CodeGraph checkout。結果包含多語言 callable 的 TP／FP／FN、precision／recall／F1、首次索引、5 次無變更同步的 median／p95／離群值、單檔同步、工作集記憶體、CLI 與 4-way MCP 延遲；SymbolLattice 另回傳不持久化的階段與子階段時間、階段邊界 RSS。版本不一致、模糊、截斷或不完整輸出會直接失敗。
- SQLite generation 寫入會重用已準備的 statement，狀態查詢不載入完整圖譜；兩者都不改變交易、active generation 或證據語意。
- `sync` 以單次 project walk 找出 scope 內來源與全專案設定候選，再以最多 8 個並行串流讀取驗證一般來源的完整 UTF-8 SHA-256；Objective-C `.h` 保留內容分類步驟。不需把所有檔案文字同時留在記憶體，也不需重建 TypeScript、workspace、Cargo、Go、Astro、Xcode 或 JVM resolver。可證明完全未變時不建立完整 scan、不載入 generation facts，也不發布新 generation；候選設定新增、刪除、內容改變或明確更換 scope 時仍走完整增量解析。
