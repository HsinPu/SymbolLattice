...
> [!IMPORTANT]
> **v0.75.0** is an early developer release. This public repository runs from source; its npm package is intentionally private and is not published to npm.
...
| Pascal | Direct column-one complete `procedure` / `function` implementations, including direct dotted and `class` routine names, emit exact file-local `contains` evidence. Comments, strings, interface-only declarations, indented routines, and incomplete source cannot create a routine symbol. |
...
| Capability | Proven surfaces in v0.75 |
| --- | --- |
...
v0.75 deliberately excludes `.dfm` / `.fmx`, forward/interface declarations, indented/local routines, constructors/destructors/operators/generics, overload/directive forms, unit/uses/module/call/type/class relationships, VCL/FMX/Lazarus form semantics, Horse/Brook/WebBroker or other framework inference, compiler proof, and runtime behavior. Unterminated comments/strings or an incomplete routine retain only a file symbol.
...
| `v0.75+` | Pascal grammar validation, `.dfm` / `.fmx` form surfaces, interface/type/class/unit/uses/call relations, local/constructor/destructor/operator/generic/overload forms, project/package resolution, Horse/Brook/WebBroker and VCL/FMX/Lazarus framework facts, compiler proof, and runtime analysis |
