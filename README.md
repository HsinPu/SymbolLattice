...
v0.489.0 加深 Java modern-parser bare call recovery。當 legacy parser 因同檔 modern syntax fail-close 時，clean modern parse 只恢復 method／constructor body 外層的 bare implicit-static／implicit-instance calls；既有 resolver 僅在同 owner、private／static／final、唯一 arity 與可通過 access／dispatch proof 時建立 `calls` exact edge。lambda／nested callable、explicit receiver、inherited virtual dispatch、overload type ambiguity、mutation／escape 與 external classpath 維持 nonclaim。
