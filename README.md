...
> [!IMPORTANT]
> v0.289.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- `explore` 同時支援精確 symbol 與有界問題模式；明示的安全專案相對路徑優先，其餘焦點候選會把 symbol／檔名證據、已選候選間的連線，以及有界 exact one-hop graph mass 乘上 persisted generated-source worth（`0.3`）與 test-source worth（`0.5`）。一般查詢只柔性降低測試檔排序，不會隱藏候選；明示測試檔或使用 `test`、`spec`、`verify` 等獨立測試意圖詞時免除 test 降權。receipt 會分開揭露原始／排序分數、兩種 classifier、意圖、權重、graph-mass 計數與上限。最多選取 4 個檔案、8 個焦點、每檔 2 個焦點與 16 條連線；問題模式只用 exact 關係尋找 4 hops 內的路徑。
...
- 索引時會保存 generated 與 production/test 來源角色證據；`files` 可直接查看分類版本與命中規則。舊 generation 不會依即時路徑靜默重判，規則版本變更會要求 `sync` 重建投影。
