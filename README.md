...
> [!IMPORTANT]
> v0.109.0 為早期開發者版本。此儲存庫從原始碼執行；npm 套件維持私有，尚未發佈。
...
- 提供 CLI 與唯讀 MCP 查詢，可查看符號、關係、路由、進入點、版本歷史、差異、受影響測試、自動同步健康、session 時間線與持久化診斷歷程。
...
- 新增 `symbol_lattice_auto_sync_journal`：唯讀查詢預設專案的持久化 watcher 歷程，可跨 MCP host 重啟讀取。
- 已初始化專案的背景 watcher 會將已安全化 receipt（含 host UUID，避免跨 host sequence 混淆）寫入 `.symbol-lattice/auto-sync-diagnostics.sqlite`；最多保留 128 筆，並回報保留、捨棄、截斷、最新持久化時間與 I/O 狀態。
- MCP request 永遠只讀取 journal；寫入僅發生在 `serve --mcp` 擁有的 watcher receipt callback。沒有 index 的專案不會因 journal 自動建立 `.symbol-lattice`。

可用 `--sync-interval <ms>` 調整輪詢備援、`--poll` 關閉原生事件加速、`--no-auto-sync` 關閉背景同步，或以 `--no-diagnostic-journal` 關閉 journal 寫入。首次使用仍需先執行 `init`；手動 `sync` 適合修復或 CI。
...
- 自動同步狀態與 session 時間線只描述目前預設 MCP host 的 watcher，不是跨程序 daemon、集中式工作佇列或效能監控系統。
- 持久化 journal 是有 128 筆上限的 operational record；它不是不可竄改 audit log、daemon lifecycle、領導者選舉或完整跨客戶端協調機制。
