...
> [!IMPORTANT]
> v0.190.0 是開發者預覽版，尚未發佈至 npm；請由原始碼執行。
...
- Android 原生模組的 getName() 現在可直接回傳類別本地的 Java static final String 或 Kotlin companion const val 常數，也保留原有字面值寫法。
- 常數必須是類別中唯一、不可變且字面的直接宣告；可變欄位、運算式、繼承值、巢狀 object 與 Codegen 基底類別都不會被推論。
...
- React Native Android 原生端接受 getName() 直接回傳的類別本地 Java static final String 或 Kotlin companion const val 字面常數；只有直接 ReactContextBaseJavaModule 繼承與直接 ReactMethod 標註仍會產生 bridge facts。
