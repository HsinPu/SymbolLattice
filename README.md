...
v0.511.0 在保留 v0.510 Pascal bounded unit relation 的前提下，加入 `.pp` content-gated discovery。`.pp` 不進 generic extension map；只有經 comment／string sanitization 後恰有一個 `unit`／`program`／`library` header，且沒有 conditional／macro／include taint 的檔案才會被標成 Pascal。300 個正向、150 個負向 discovery fixtures，以及 Puppet／`.inc`／假 header／multiple／tainted／malformed negatives 均通過，既有 `.pas`／`.lpr`／`.dpr`／`.dpk` 行為不變。Pascal relation 的 overload／ambiguous exports、qualified/member／virtual dispatch、forward／external、package resolution 與 runtime semantics 仍是 nonclaim；`.pp` admission 也不代表 Puppet 支援。
...
v0.505.0 將Solidity提升為bounded same-file relation evidence。固定OpenZeppelin Contracts v5.7.0、PRBMath v4.2.0與forge-std v1.16.2，以solc 0.8.31 AST驗證300筆same-contract private fixed-arity calls、完整328筆candidate breadth及150筆負向。Exact `calls`只接受同一contract中唯一`private`函式的bare identifier呼叫、參數數量一致、無lexical shadow、function-typed value或caller inline assembly；internal／public／external target、overload、qualified/member call、library、package resolution與runtime dispatch維持nonclaim。

v0.504.0 將Lua提升為bounded same-file relation evidence。固定LuaRocks 3.13.0、Kong 3.9.3與Lua 5.5.1 corpus驗證300筆tree-sitter parser-proven direct calls及150筆負向。Exact `calls`只接受direct-root function body中的bare identifier、同檔唯一且較早宣告或self-recursive的`local function` target、無parameter/local/assignment shadow與固定UTF-8 byte range；global／dotted／colon dispatch、nested function、`debug`、dynamic load、`require` module resolution、metatable與runtime dispatch維持nonclaim。
...
v0.489.0 加深 Java modern-parser bare call recovery。當 legacy parser 因同檔 modern syntax fail-close 時，clean modern parse 只恢復 method／constructor body 外層的 bare implicit-static／implicit-instance calls；既有 resolver 僅在同 owner、private／static／final、唯一 arity 與可通過 access／dispatch proof 時建立 `calls` exact edge。lambda／nested callable、explicit receiver、inherited virtual dispatch、overload type ambiguity、mutation／escape 與 external classpath 維持 nonclaim。固定三 corpus relation selection 為 241 TP／59 FN、0 FP、0 evidenceInvalid，150／150 負向通過；6 筆 call truth 回補。

v0.490.0 加深 Java modern-parser parameter receiver recovery。對 clean modern parse 中直接 declared reference-type parameter 的 `param.method(...)`，只有在參數未被 reassignment／escape／lambda capture 污染、method target 可由既有 resolver 唯一證明時建立 `calls` exact edge；field receiver、chained／computed receiver、array／wildcard type、inherited virtual dispatch 與 overload type inference 維持 nonclaim。固定三 corpus relation selection 為 242 TP／58 FN、0 FP、0 evidenceInvalid，150／150 負向通過；相較 v0.489 新增 1 個 incremental TP。
...
v0.494.0 延伸 Java modern-parser local initializer receiver：對 clean modern parse 中 direct `DeclaredType value = new ConcreteType(...)`，只有 declared type 與 concrete type 皆為單一非 generic identifier，且 resolver 能以一條 tracked exact `extends`／`implements` edge 證明 assignability 時才產生 `calls`；未解析的外部 type、間接 heritage、generic／wildcard／array／anonymous／ambiguous initializer 仍維持 unresolved。既有 v0.493 generic raw-type 支援不變；固定三 corpus benchmark 與 150 個負向結果會在 release report 中公布。
...
v0.475.0 加深 PHP bounded project relations。固定 Laravel framework 12.0.0、Symfony 7.3.0、Composer 2.8.10 與 PHPUnit 12.2.0 四個 production PHP scope，加入 namespace／`use` import、class heritage、唯一 direct function／static method call、class instantiation，以及 typed signature accepts／returns；300 個 synthetic 正向為 TP300／FP0／FN0／evidenceInvalid0，150 個 disposable 負向全數 fail-close。autoload／Composer dependency、traits／magic method、closures／anonymous classes、dynamic call、overload／attribute runtime、reflection、generated code、external package 與 malformed parser recovery 維持 unresolved 或 nonclaim。四個 corpus 的 parser-admitted breadth 另列；Windows 沒有 PHP compiler／Composer runtime validation，因此不宣稱完整 PHP 語意。
