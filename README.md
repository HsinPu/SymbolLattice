...
> [!IMPORTANT]
> v0.303.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 可沿直接的 `Factory.create().execute()` 靜態 factory chain，以已證明的 top-level 回傳型別解析兩段 exact `calls`。fixed／varargs 先依 argument count 篩選，再使用 primitive／string literal、明示 primitive cast 或非泛型 `new Type(...)` 的型別證據。primitive identity/widening 與專案內 exact `extends`／`implements` reference widening 會共同參與 overload selection；reference 候選依 parameter specificity 比較，不只看 argument 距離。若回傳型別沒有同名方法，還會沿 exact hierarchy 選出唯一最具體的 inherited class 或 interface owner，包含合法的 default interface method。所有 hierarchy traversal 都限制為 16 hops、256 types，超限或 owner 歧義即保持不解析；`callType` 與 `callDispatch` receipt 揭露選擇原因、候選 owner、bounds 與逐段 edge。未知變數、`null`、boxing/unboxing、reference cast、泛型／array、instance factory、shadowed receiver、跨 owner overload 合併及互不相干 class／interface precedence 仍不會被猜成精確 edge。
