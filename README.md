...
> [!IMPORTANT]
> v0.295.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- TypeScript 函式與類別方法的輸入／回傳型別會形成 exact `accepts`／`returns` 關係；只接受可證明的本機、type-only import 或 re-export 型別，泛型參數、內建包裝型別、qualified name 與未匯入同名型別不會被猜成精確關係。
