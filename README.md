...
> [!IMPORTANT]
> v0.418.1 是開發預覽版。MCP 查詢工具本身唯讀；`serve --mcp` 預設會另外啟動本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用自動同步。
...
- Handler exactness 僅涵蓋 literal POST form/button、唯一 conventional `.cshtml.cs` companion、source-proven `PageModel`，以及唯一公開、非靜態、非泛型、非 override、未標記 `NonHandler` 的 `OnPost…` 方法。按鈕的 literal `formmethod` 會覆寫父表單方法；只有省略或明確為 `post` 才保留 exact handler。動態 Tag Helper 值、跨頁 handler、partial/overload、runtime model binding、authorization 與 Razor rendering lifecycle 均不宣稱。extractor facts 為 v276，resolver 為 v144。
