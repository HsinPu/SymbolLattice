...
- `explore` 同時支援精確 symbol 與有界問題模式；明示的安全專案相對路徑優先，其餘焦點候選會把 symbol、檔名與精確圖譜連線證據乘上 persisted generated-source worth（`0.3`）。這是柔性的數值排序，不會隱藏 generated 檔；明確指定的 generated 路徑不受降權。receipt 會分開揭露原始分數、排序分數與 classifier 證據。最多選取 4 個檔案、8 個焦點、每檔 2 個焦點與 16 條連線；問題模式會在最多 16 組焦點間尋找 4 hops 內的 exact path，保留最多 4 條 spine 與 8 個未選取的橋接 symbol，不會用 heuristic 或 unresolved 關係補路徑。
