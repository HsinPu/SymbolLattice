...
> [!IMPORTANT]
> v0.311.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 明示的 `this.method()`、`super.method()`、具有直接 reference type 的 parameter／block-local／同類別 field receiver，以及 `var receiver = new DirectType(...)`，會重用相同的 overload、method-set 與 access 證據。欄位支援裸名稱與 `this.field`，保存型別、宣告／類別範圍、visibility 與 static 證據；lexical shadowing、static context、重名或不支援型別會 fail closed。`var` 只接受非泛型直接 object creation；繼承欄位、factory call、一般賦值、scope 外、泛型及任意 expression receiver 仍保持不解析。`super` 仍要求唯一 exact 直接父類並保留完整路徑。
