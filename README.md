...
> [!IMPORTANT]
> **v0.87.0** is an early developer release. This public repository runs from source; its npm package is intentionally private and is not published to npm.

> [!NOTE]
> **v0.87.0 — parser-proven YAML top-level scalar keys.** SymbolLattice now indexes `.yaml` and `.yml` files, retaining only source-ranged, untagged, unanchored top-level scalar mapping keys from one valid YAML document.
...
- **YAML scalar-key evidence** - a valid single YAML document can contribute source-ranged top-level scalar mapping keys as auditable variables, while nested structures and YAML features that do not provide a direct scalar key/value proof remain outside the graph.
...
This produces `variable` symbols for `service`, `port`, and `owner`; each has a direct `contains` edge with `syntax.yaml.top-level-scalar-mapping` evidence. Nested maps and sequences remain indexed source text but do not become derived symbols or relationships.

The parser rejects malformed and multi-document streams as a whole. Valid but outside-slice constructs such as nested mappings/sequences, aliases, anchors, explicit tags, complex keys, block scalars, imports, calls, framework routes, cross-file configuration semantics, schema validation, deployment behavior, and runtime values deliberately produce no YAML declaration fact. This avoids treating configuration structure as executable control flow.
...
- The YAML surface requires one parser-valid document with a top-level mapping, and emits only source-ranged untagged, unanchored scalar-key/scalar-value pairs that stay on one line. It excludes nested mappings/sequences, empty or null values, aliases, anchors, explicit tags, block scalars, complex keys, multi-document streams, configuration merging, schema validation, imports, calls, routes, framework semantics, cross-file configuration resolution, deployment behavior, and runtime values. Parser errors retain only a file symbol.
...
| `v0.87+` | YAML source kinds beyond top-level scalar mapping entries, aliases/anchors/tags/merge semantics, complex keys and block scalars, schema validation, configuration references, Kubernetes/Compose/CI framework packs, cross-file configuration resolution, deployment semantics, and runtime analysis |
...
YAML coverage includes `.yaml` / `.yml` discovery, parser-validated single-document input, persisted source search and CLI/MCP language filters, source-ranged top-level untagged/unanchored scalar mapping-key containment, nested collection exclusion, malformed/multi-document fail-closed behavior, and no import, call, route, schema, merge, cross-file, deployment, or runtime claim.
