...
> [!IMPORTANT]
> v0.331.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 否定 pattern guard 支援 `if (!(value instanceof DirectType name))`。若 then 分支以可證明的 top-level `return` 或 `throw` 結束，v14 會把 binding 延伸至同 lexical block 的後續作用域；有 `else` 時，v15 會在 else body 內建立 definitely-matched binding，並只在 then 分支可證明 abrupt completion 時繼續延伸到 if 之後。v16 另支援無標籤 `break`／`continue` guard；v17 將同一個精確 loop-target 證據延伸到 negated `else`，讓 else body 與 if 後續作用域各自保存 active region。兩者只接受 guard 位於可精確證明的 while、do、for 或 enhanced-for 實體 block 內，並保存 loop target kind/range。receipt 會保存 guard、condition、pattern、最多 4 層括號、body、abrupt statement、target、active region 與精確 scope。fall-through、條件式 exit、labeled jump、無大括號 loop、switch-break、同名既有 binding、泛型、錯誤極性與區塊外都保持不解析。
