...
目前版本：**v0.527.0** · Node.js **>=22.13 <25** · [MIT](LICENSE)

## 能做什麼

透過 CLI 或 MCP 查詢 repository，索引保存在本機 `.SymbolLattice/`。

- 找出任務相關的檔案與符號，附上原始碼、行號和關係證據；保留 HTTP 500 這類數字查詢詞，並限量補上實際命中位置的來源片段。
- 追蹤呼叫、繼承、匯入與框架入口，協助評估修改影響。
- 增量更新索引，查看歷史與差異。

採靜態分析；動態呼叫、反射、巨集與外部套件可能無法解析。`explore` 會限量附上索引已記錄的未解析呼叫與來源位置；Python 靜態點號成員呼叫可保留這類證據，但不推定接收者型別或目標。同名宣告可作補充追查，但仍明示呼叫目標未解析。未解析、未記錄或被截斷的結果不代表相關程式碼不存在。

查詢範圍、限制與實測結果見[驗證文件](benchmarks/README.md#numeric-query-and-unresolved-name-evidence)。
...
Python 可查詢直接模組賦值的變數名稱與來源；不代表已解析其執行期值或別名目標。
