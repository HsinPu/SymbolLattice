...
**Evidence-first local code intelligence for TypeScript, JavaScript, Python, Go, Rust, Java, PHP, C++, C#, Ruby, Kotlin, Swift, Dart, and Scala projects.**
...
> [!IMPORTANT]
> **v0.45.0** is an early developer release. This public repository runs from source; its npm package is intentionally private and is not published to npm.
...
| Area | v0.45.0 behavior |
| --- | --- |
| Source files | TypeScript, TSX, JavaScript, JSX, Python, Go, Rust, Java, PHP, C++, C#, Ruby, Kotlin, Swift, Dart, and Scala (`.cpp`, `.cc`, `.cxx`, `.hpp`, `.hh`, `.hxx`, `.cs`, `.rb`, `.kt`, `.swift`, `.dart`, `.scala`; plus Play `conf/routes` and `conf/*.routes` route tables) |
...
| Capability | Proven surfaces in v0.45 |
| --- | --- |
...
| Play | Scala direct `conf/routes` / `conf/*.routes` literal controller-action entries with explicit unresolved handlers |
...
v0.45 adds Scala `.scala` discovery, direct top-level class/object/trait/method/function containment, and a first `play` capability. Play route tables are a separate configuration syntax, so SymbolLattice also discovers only `conf/routes` and `conf/*.routes` as Scala-owned inputs. It accepts one literal HTTP verb, one slash-prefixed path, and one controller-action reference:
...
This emits `GET /health` and `POST /orders` route symbols with `framework.play.conf-routes.literal-controller-action.unresolved-handler` evidence. The route path and controller-action spelling are static and auditable, but the controller lives in a different Scala file, so v0.45 deliberately keeps the `routes` edge `unresolved` rather than guessing a target.

It intentionally excludes `->` included routers, nonliteral or malformed route rows, route entries outside `conf/routes` / `conf/*.routes`, route prefixes/composition, `build.sbt` project detection, controller/package/import/classpath resolution, overloads, custom route binders, reverse routing, and runtime Play behavior. Scala files with syntax errors retain only their file symbol until repaired.
...
flowchart LR
  ...
  Extractor["TypeScript AST extractor + Python/Go/Rust/Java/PHP/C++ Lezer parsers + C#/Ruby/Kotlin/Swift/Dart/Scala ast-grep parsers\nfirst-party framework capability passes\nExpress/Fastify/Nest HTTP routes + Fastify plugin facts + FastAPI/Flask/Gin/net-http/Chi/Axum/Spring Web/Laravel/cpp-httplib/ASP.NET Core/Rails/Ktor/Vapor/Flutter/Play facts\nFlutter/React Router/Next client navigation\nNest module-prefix facts + non-HTTP entrypoints"] --> Facts["Reusable artifact facts"]
...
src/
  ...
  extraction/      TypeScript AST plus Python, Go, Rust, Java, PHP, C++ Lezer and C#/Ruby/Kotlin/Swift/Dart/Scala ast-grep fact extraction
...
- The Scala surface proves direct top-level classes, objects, traits, methods, and functions through the Scala AST. The Play surface proves only literal `conf/routes` / `conf/*.routes` controller-action entries and records every accepted handler as `unresolved`. It excludes `->` includes, prefixes/composition, `build.sbt` detection, controller/package/import/classpath or overload resolution, custom binders, reverse routing, Scala 3 contextual declarations, generic Scala call/type analysis, and runtime Play behavior. Syntax-error Scala files retain only their file symbol until repaired.
...
- Language adapters beyond TS/TSX/JS/JSX/Python/Go/Rust/Java/PHP/C++/C#/Ruby/Kotlin/Swift/Dart/Scala, external dependency indexing, telemetry, or multi-project routing.
