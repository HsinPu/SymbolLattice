...
- `impact` 現在回傳 `summary`：依受影響終點所在檔案分組，保留最短深度、終點符號與發現它的最後一條邊。
- `summary.entrypointCoverage` 只列出確實作為保留路徑終點的 HTTP route 與非 HTTP entrypoint 記錄；不會因為同檔案或中途經過而推論涵蓋。
- 新增唯讀 MCP 工具 `symbol_lattice_impact`。它透過 MCP query worker 執行，深度限制為 1–3 跳、最多回傳 100 條路徑；`truncated: true` 表示摘要並不完整。

## 範圍與限制

- 這是本機程式碼圖譜，不是 RDF／SPARQL 知識圖譜或本體推理系統。
- 圖譜查詢只讀取已保存的世代。`investigate --ranking impact` 的分數只用受限的 `exact` 靜態證據；一般 `impact` 查詢則沿用既有的已解析靜態關係，摘要不會升級或混淆邊的信心等級。
