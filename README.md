...
Java 深度包含唯一專案型別的明確 import、annotation、泛型 direct heritage／object creation、parser-recovery callable signature／bare call／parameter receiver，以及 `build.gradle(.kts)` 或 module-named Gradle build script 證據；重複 qualified type、wildcard／static import、lambda 內建立、array／wildcard signature、mutation／escape、anonymous interface 與外部 classpath 仍保守省略。
