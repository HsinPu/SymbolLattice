...
> [!IMPORTANT]
> v0.305.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 可沿直接的 `Factory.create().execute()` 靜態 factory chain，以已證明的 top-level 回傳型別解析兩段 exact `calls`。method set 會合併 direct 與 inherited overload，並依 Java owner precedence 選擇；target method 支援 public、同 package 的 package-private／protected，以及可證明 caller-subclass 與 receiver constraint 的跨 package protected 存取。`callType`、`callDispatch` 與 access receipt 會揭露 signature、owner、package、bounds 與逐段 hierarchy edge。factory 仍限 public static；private、跨 package package-private、無法證明的 protected、未知型別、boxing／泛型等情況保持不解析。
