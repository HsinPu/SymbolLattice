...
> [!IMPORTANT]
> v0.162.0 是開發者預覽版，尚未發佈到 npm；請由原始碼執行。
...
- 新增 Django `re_path` 直接路由萃取，支援匯入別名、raw 字串與一般單行字串。
- 僅當 pattern 是完整錨定的 `^靜態路徑$` 時，才會建立 `exact` 的 `ALL` 路由與本地函式 handler 關係。
- 捕捉群組、萬用字元、逸出字元、缺少錨點、動態值與重綁定不會被猜測為精確路由。
...
- 跨檔 Python 路由目前涵蓋 FastAPI `include_router`、Flask `register_blueprint`、Sanic `app.blueprint` 與 Django `path(..., include(...))`。
- Django 直接 `re_path` 僅接受單一路徑可完整比對的字面 pattern；正規表示式語意與 `re_path` 的跨檔掛載尚不產生 `exact` 結果。
