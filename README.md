...
> [!IMPORTANT]
> v0.309.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 明示的 `this.method()`、`super.method()`，以及具有直接 reference type 的 method parameter／block-local receiver，會重用相同的 overload、method-set 與 access 證據。parameter／local binding 以 lexical scope、宣告順序、型別與來源範圍綁定 active generation；未宣告、宣告前、scope 外、欄位、泛型或任意 expression receiver 不會被猜成 exact edge。`super` 仍要求唯一 exact 直接父類，並保留 caller-to-super path、繞過子類 override。
