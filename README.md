...
> [!IMPORTANT]
> v0.314.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 明示的 `this.method()`、`super.method()`、具有直接 reference type 的 parameter／block-local／field receiver，以及 `var receiver = new DirectType(...)`，會重用相同的 overload、method-set 與 access 證據。欄位支援裸名稱、`this.field` 與 `super.field`；一般欄位先採最近 class 宣告，再沿 exact `implements`／interface `extends` 圖查找 interface constant，遵守 subinterface hiding、class precedence 與歧義 fail-closed。介面常數會保存隱含 `public static final` 證據；`super.field` 仍只查 class 父鏈。每次解析都保存 owner path、型別種類、modifier proof、visibility、package access 與 bounds。lexical shadowing、不可存取或未知型別的最近宣告、循環／多重 owner、界限與 static context 違規都 fail closed。`var` 只接受非泛型直接 object creation；qualified interface field、nestmate private、一般賦值、泛型／array 欄位與 classpath 推測仍保持不解析。`super.method()` 仍要求唯一 exact 直接父類並保留完整路徑。
