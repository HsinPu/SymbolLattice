...
> [!IMPORTANT]
> v0.299.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 可沿著直接的 `Factory.create().execute()` 靜態 factory chain，以已證明的 top-level 回傳型別解析兩段 exact `calls`。固定參數與 varargs overload 只有一個宣告符合實際 argument count 時可精確選取，edge receipt 會列出所有候選、arity 範圍與適用判定；相同 arity、重疊 varargs、instance factory、shadowed receiver、wildcard／未匯入型別、參數型別轉換及繼承方法都保持不解析。
