...
> [!IMPORTANT]
> v0.342.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- `benchmark:comparison` 會在隔離的 Git 工作樹副本上，以同一組真值與查詢比較 SymbolLattice 與指定 CodeGraph checkout。結果包含 TP／FP／FN、precision／recall／F1、未解析率、首次索引、無變更同步、單檔同步、工作集記憶體、CLI 延遲與 4-way MCP 並行延遲；SymbolLattice 另回傳不持久化的階段時間與階段邊界 RSS 收據。版本不一致、模糊、截斷或不完整輸出會直接失敗。
