...
> [!IMPORTANT]
> v0.300.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 可沿著直接的 `Factory.create().execute()` 靜態 factory chain，以已證明的 top-level 回傳型別解析兩段 exact `calls`。固定參數與 varargs 先依實際 argument count 篩選；同 arity 候選只有在 primitive／string literal 或非泛型 `new Type(...)` 能證明一個 exact identity 相容、其餘均不相容時才選取。`callArity`／`callType` receipt 會列出所有候選、範圍、ordered types 與相容性；未知變數、`null`、boxing、widening、subtyping、cast、泛型／array 型別、instance factory、shadowed receiver 及繼承方法不會被猜成型別精確 edge。
