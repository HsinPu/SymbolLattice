...
> [!IMPORTANT]
> v0.100.0 為早期開發者版本。此儲存庫從原始碼執行；npm 套件維持私有，尚未發佈。
...
- 支援前端、後端、JVM、原生語言、資料格式、IaC、模板與 schema 檔案；包含 TypeScript、Java、Groovy、Python、Go、Rust、C/C++、C#、PHP、Ruby、Kotlin、Swift、Dart、SQL、GraphQL、Protocol Buffers、Terraform、YAML、XML 等。
...
- 新增 `.groovy` 掃描：完整頂層 `class`、`interface`、`trait`、`enum` 與 `def name(...) { ... }` 會產生帶來源範圍的符號及精確 `contains` 證據。
- `trait` 使用既有 `interface` 符號類型表示；註解、一般／三引號字串與註解文字不會產生假符號。
- README 改為預設繁體中文，並提供對等精簡英文版。
