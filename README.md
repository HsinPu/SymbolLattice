...
> [!IMPORTANT]
> v0.187.0 是開發者預覽版，尚未發佈至 npm；請由原始碼執行。
...
- 新增 React Native TurboModule：可辨識直接匯入或命名空間匯入的 `TurboModuleRegistry.get*`，並連結不可變 Registry 結果的 JS/TS 方法呼叫。
- 已匯出的 `interface Spec extends TurboModule` 會產生可查詢的方法符號；當一個字面 Registry 註冊能證明模組名稱時，規格方法也會連到原生實作。
...
- React Native 目前涵蓋嚴格的 `NativeModules` 與直接 TurboModule Registry／TypeScript 規格；跨檔預設匯入、Codegen 產生的原生基底類別、執行期註冊、間接或動態名稱、Swift 與自訂 macro wrapper 仍不在範圍內。
