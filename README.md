...
> [!IMPORTANT]
> v0.252.0 是從原始碼執行的開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
跨檔案 finalizer 使用 `createFrameworkProjectPluginRegistry` 與 `frameworkProjectPlugins`；它只讀取已抽取 facts，可回傳受限 reference 或有來源範圍的 route-prefix 描述。外掛不能直接寫入 raw graph identity。單檔外掛版本變更會重新抽取 facts；project finalizer 版本變更只重做 projection。需要處理仍未解析的目標時，可再組合 `ReferenceResolverPlugin`。
