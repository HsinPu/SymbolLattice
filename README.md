...
> [!IMPORTANT]
> v0.328.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 否定 pattern guard 支援 `if (!(value instanceof DirectType name))` 後接已證明不會正常完成的 `return` 或 `throw`。v14 receipt 保存 guard、condition、pattern、最多 4 層括號、exit body、abrupt statement 與同 lexical block 的後續作用域；區塊 body 必須以 top-level abrupt statement 結尾。fall-through、條件式 exit、`else`、同名既有 binding、泛型、錯誤極性與區塊外都保持不解析。
