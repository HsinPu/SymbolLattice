...
> [!IMPORTANT]
> v0.321.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 未初始化的明確型別區域變數，可在同一 lexical block 的直接 `receiver = new DirectType(...)` 賦值後作為 member-call receiver；也支援只有兩個 block 分支、且兩側各自完成一次直接建立物件賦值的完整 `if/else` 合流。宣告型別與每個建立型別必須相同，或由 exact hierarchy 證明 reference widening；缺少 `else`、`else if`、額外分支敘述、型別不相容及後續重新賦值都 fail closed。v6/v7 receipt 保存宣告、scope、statement、condition、每個分支的賦值與 initializer range、canonical types、相容性決策及完整 hierarchy path。
