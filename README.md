...
> [!IMPORTANT]
> v0.175.0 是開發者預覽版，尚未發佈至 npm；請由原始碼執行。
...
- Spring `@Value` 現可從 Java／Kotlin 直接類別內具實體 body 的方法參數建立設定關係，且仍以擁有方法的類別作為圖譜來源。
- 欄位、建構子與方法三種注入面共用相同證據：精確鍵為 `exact`、唯一 relaxed spelling 為 `heuristic 0.75`、碰撞保持 `unresolved`。
...
- Spring Boot `@Value` 僅連結 Java 直接頂層類別的欄位、建構子參數或具實體 body 的方法參數，或 Kotlin 直接頂層類別的屬性、primary constructor 參數或具實體 body 的方法參數；必須是精確 import／完整名稱與單一字面 placeholder，且 Kotlin 必須使用跳脫 `$` 的規則字串。`@ConfigurationProperties` 僅接受單一字面位置參數或 `prefix =` 參數。abstract／介面／top-level 方法、secondary constructor、use-site target、別名／wildcard import、欄位／集合綁定、列表、合併鍵、動態值、設定匯入、profile 啟用與執行時優先順序都不支援。
