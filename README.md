...
v0.517.0 加深 Go 具體 receiver 的直接方法呼叫：區分方法呼叫結果與 receiver 本身的傳參，避免註解、字串與相等比較被誤判為外傳或賦值。四個固定大型專案的 compiler-confirmed 方法呼叫 TP 從 103 提升至 180／581；仍有 401 筆漏失。150 筆負向契約通過，但新增關係仍有未經 compiler 驗證的部分，不代表完整 Go 支援或完整 FP=0。介面派發、method value、欄位傳參與閉包等保守邊界不放寬。
...
| 類別 | 語言與格式 |
| --- | --- |
| 已有大型專案驗證（部分例子；範圍各異） | TypeScript、Java、Go、HTML、Markdown、CSS、JavaScript、JSP、Python、Ruby、Shell、Lua、Luau、Julia、Perl、R、Elixir、Objective-C |
...
58 種語言與格式都有掃描入口，但宣告、同檔關係、跨檔關係與 framework 語意的驗證範圍不同。非空小語料用來驗證基本事實輸出；大型專案的獨立真值用來衡量關係覆蓋率，兩者分開計算。

- TypeScript／Java 等具備外部真值驗證；仍保留 overload、外部依賴與動態 dispatch 等限制。
- Go v0.513 的固定 300 筆真值找到 153 筆，全候選找到 452／986；150 筆負向通過。正向召回評分沒有量測整體 FP，不能據此宣稱完整精確率。
- Vue／Svelte／Astro 的组件引用、Python 的 direct self calls，以及 Shell／Lua 的同檔呼叫均有明確的靜態契約；動態載入、mutation 與 framework runtime 不在該契約內。
- HTML／CSS／文件／設定格式以適用的 resource、containment 與靜態引用驗證，不套用函式呼叫覆蓋率。
- ArkTS、Razor、Blade、Terraform、Liquid、Twig、CFML、YAML、XML 與 Properties 仍以針對性測試為主，沒有相同規模的標準化關係驗證。

解析器拒絕、宣告端點缺失、未解析關係與核准樣本會分別統計。較小樣本通過不代表整個語言或大型 repository 完整支援。
