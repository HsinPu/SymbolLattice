...
> [!IMPORTANT]
> v0.258.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- `files` 僅查詢 active generation 已保存的檔案，支援 anchored glob、flat/tree/grouped 投影與安全游標分頁；游標綁定 generation 與篩選條件，並明示剩餘筆數、樹深限制、圖譜計數與 freshness。
