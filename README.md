...
> [!IMPORTANT]
> v0.137.0 是開發者預覽版，尚未發布到 npm；請從原始碼執行。
...
- GoFrame v1/v2 現支援同一函式內未重綁的 controller 指標短宣告：`controller := &Controller{}` 或 `controller := new(Controller)` 後，`Bind(controller)` 可產生 `exact` route，且能與同一個 variadic `Bind(...)` 的其他已證明 argument 並存。
- controller alias 保留既有 direct Bind 的 `Group` 前綴、`Domain` host 與根 `go.mod` 跨套件證據；重新賦值與 callback 同名參數會使 alias 失效。
- artifact facts 升級為 `multi-language-ast-v118`；下次明確 `sync` 會重新萃取 Go facts 並重投影路由。
...
- GoFrame 跨檔 standard router 僅支援可靜態證明的 direct pointer（`&Controller{}`／`new(Controller)`）、無參數 `Factory()`，及同一函式內未重綁的一層 pointer／factory short-declaration alias。每個 `Bind(...)` argument 必須獨立成立；slice 展開、動態值、`var`／全域、轉送、分支、map/interface/DI 容器、callback 同名遮蔽、重綁與歧義不會成為 `exact`。顯式 import alias 可直接使用；預設 import 必須由目標 `package` 宣告證明，絕不從 import 路徑猜測。`.`／`_` import、外部/傳遞模組、`replace`、巢狀模組選擇與 build tag 保持未解析。
