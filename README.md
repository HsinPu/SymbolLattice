...
> [!IMPORTANT]
> v0.291.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- `explore` 同時支援精確 symbol 與有界問題模式；明示的安全專案相對路徑優先，其餘候選會結合 lexical、連線與 exact one-hop graph mass，再套用 persisted generated worth（`0.3`）及 test／icon／localization worth（各 `0.5`）。一般問題只有在至少 2 個不同候選檔被當前分類器證明為 production 時，才排除未要求的低價值角色；證據不足時只柔性降權。角色意圖與明示檔案會個別豁免，receipt 提供逐角色計數與最多 16 個排除檔案的證據。選取仍限制為 4 個檔案、8 個焦點、每檔 2 個焦點、16 條連線與 4 hops exact path。
