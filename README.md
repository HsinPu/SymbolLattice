...
> [!IMPORTANT]
> v0.312.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 明示的 `this.method()`、`super.method()`、具有直接 reference type 的 parameter／block-local／field receiver，以及 `var receiver = new DirectType(...)`，會重用相同的 overload、method-set 與 access 證據。欄位支援裸名稱與 `this.field`，可沿唯一 exact class `extends` 鏈選出最近宣告，並保存 owner path、visibility、package access 與 static 證據；lexical shadowing、不可存取或未知型別的最近宣告、循環／多重 owner、界限與 static context 違規都 fail closed。`var` 只接受非泛型直接 object creation；interface／`super.field`、nestmate private、一般賦值、泛型欄位、array 與 classpath 推測仍保持不解析。`super.method()` 仍要求唯一 exact 直接父類並保留完整路徑。
