...
目前版本：**v0.525.0** · Node.js **>=22.13 <25** · [MIT](LICENSE)
...
採靜態分析；動態呼叫、反射、巨集與外部套件可能無法解析。`explore` 會限量附上索引已記錄的未解析呼叫與來源位置；Python 靜態點號成員呼叫可保留這類證據，但不推定接收者型別或目標。未解析、未記錄或被截斷的結果不代表相關程式碼不存在。
