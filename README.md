...
> [!IMPORTANT]
> v0.334.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 否定 pattern guard 支援 `if (!(value instanceof DirectType name))`。v14／v15 處理可證明的 `return`／`throw` 與 negated `else`，v16–v19 處理無標籤及具名 loop exit，v20／v21 處理傳統 `case:` 群組內的 `switch break`。v22／v23 新增 switch expression arrow-rule block 的 top-level `yield`，可證明其位於 `return`、變數 initializer、assignment 或外層 `yield`；receipt 保存 switch body、rule、rule body、label、使用位置與精確 scope。傳統 case binding 不跨下一個 label，arrow-rule binding 不離開目前 rule block；巢狀 loop 仍接管最近的 `break`。冒號式 `yield`、statement switch、條件式 exit、不安全巢狀 guard、同名既有 binding、泛型、錯誤極性與作用域外都 fail closed。
