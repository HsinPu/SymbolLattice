...
> [!IMPORTANT]
> v0.346.0 是開發預覽版。MCP 查詢工具本身唯讀；`serve --mcp` 預設會另外啟動本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用自動同步。
...
- TypeScript、JavaScript、Java、Python、Go、Rust、C、C++ 已能在最小案例建立經來源、目標與 edge ID 精確核對的基本 call 關係；其餘六種優先語言可掃描與查詢，但 call 關係深度仍待加強。
- Go、Rust、C、C++ 只在同檔、唯一、未遮蔽的 top-level/free function 裸呼叫建立 exact edge。parameter/local shadow、closure/lambda、member/qualified、macro、overload與間接呼叫等不確定形式會保守拒絕。
