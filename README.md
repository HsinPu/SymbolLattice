...
> [!IMPORTANT]
> v0.339.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- `sync` 會先以檔案內容雜湊、設定與 indexer 版本做輕量預檢；可證明完全未變時不載入完整 generation facts，也不發布新 generation。無法證明時仍走完整增量解析。
