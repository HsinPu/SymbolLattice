...
> [!IMPORTANT]
> v0.176.0 是開發者預覽版，尚未發佈至 npm；請由原始碼執行。
...
- Spring `@Value` 現可直接標記在 Java／Kotlin 具實體 body、唯一參數的方法上；關係仍由擁有方法的類別發出。
- 若該唯一參數已有可證明的 Spring `@Value`，參數證據優先，避免重複或衝突關係；其餘 exact／relaxed／collision 規則不變。
...
- Spring Boot `@Value` 僅連結 Java 直接頂層類別的欄位、建構子參數、具實體 body 的方法參數，或具實體 body 且唯一參數的方法本身；Kotlin 具有相同的屬性、primary constructor、方法參數與方法註解範圍。必須是精確 import／完整名稱與單一字面 placeholder，且 Kotlin 必須使用跳脫 `$` 的規則字串。若參數已有可證明的 `@Value`，方法註解不重複發出關係。`@ConfigurationProperties` 僅接受單一字面位置參數或 `prefix =` 參數。abstract／介面／top-level 方法、secondary constructor、use-site target、別名／wildcard import、欄位／集合綁定、列表、合併鍵、動態值、設定匯入、profile 啟用與執行時優先順序都不支援。
