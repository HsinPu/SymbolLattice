...
> [!IMPORTANT]
> v0.306.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 可沿直接的 `Factory.create().execute()` 靜態 factory chain，以已證明的 top-level 回傳型別解析兩段 exact `calls`。factory 支援 public static 與 caller 同 package 的 package-private／protected static；target method 另支援可證明 caller-subclass 與 receiver constraint 的跨 package protected 存取。`callType`、`callDispatch` 與 access receipt 會揭露 signature、owner、package、bounds 與逐段 hierarchy edge。private、跨 package package-private、未證明的 inherited protected static、未知型別、boxing／泛型等情況保持不解析。
