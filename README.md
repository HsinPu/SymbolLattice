...
> [!IMPORTANT]
> v0.308.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 明示的 `this.method()` 與 `super.method()` 會重用相同的 overload、method-set 與 access 證據。`super` 只在唯一 exact 直接父類可證明時解析，並保留 caller-to-super path、繞過子類 override；區域變數、欄位與任意 expression receiver 仍不會被猜成 exact edge。
