...
> [!IMPORTANT]
> v0.301.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 可沿直接的 `Factory.create().execute()` 靜態 factory chain，以已證明的 top-level 回傳型別解析兩段 exact `calls`。固定參數與 varargs 先依實際 argument count 篩選；之後以 primitive／string literal、明示 primitive cast 或非泛型 `new Type(...)` 的型別證據選取 overload。primitive identity 與 Java widening（`byte → short → int → long → float → double`、`char → int → long → float → double`）採 fixed-arity phase 優先及逐參數 Pareto 最佳規則，沒有唯一最佳候選就保持不解析。`callArity`／`callType` receipt 會列出 invocation mode、每個 conversion、distance 與最後選取；未知變數、`null`、boxing、unboxing、reference subtyping／cast、泛型／array 型別、instance factory、shadowed receiver 及繼承方法不會被猜成精確 edge。
