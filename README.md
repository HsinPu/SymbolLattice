...
`v0.528.21` · Node.js `>=22.13 <25` · MIT
...
SymbolLattice 使用靜態分析。文字命中也可能來自註解或字串，須查看來源確認語意。動態呼叫、反射與外部依賴可能無法解析，同名宣告也不代表已確認的呼叫目標。結果有數量與來源片段上限；未找到關係不能用來保證修改或刪除安全。索引過期時請先同步，無法確認新鮮度的即時查詢可能拒絕回傳結果。
