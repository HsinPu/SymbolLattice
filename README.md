...
> [!IMPORTANT]
> v0.188.0 是開發者預覽版，尚未發佈至 npm；請由原始碼執行。
...
- 新增跨檔 TurboModule 預設匯入：`import Calendar from "./NativeCalendar"` 的直接方法呼叫，能連回已證明的本機 TurboModule 預設匯出。
- 目標檔必須直接匯出字面的 Registry 結果或不可變 Registry 綁定；一般預設匯入不會被誤當成 React Native bridge。
...
- React Native 目前涵蓋嚴格的 `NativeModules`、直接 TurboModule Registry／TypeScript 規格，以及已證明本機預設匯出的跨檔使用端；預設匯出再轉出、Codegen 產生的原生基底類別、執行期註冊、間接或動態名稱、Swift 與自訂 macro wrapper 仍不在範圍內。
