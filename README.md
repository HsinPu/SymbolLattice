...
> [!IMPORTANT]
> v0.310.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 明示的 `this.method()`、`super.method()`、具有直接 reference type 的 parameter／block-local receiver，以及 `var receiver = new DirectType(...)`，會重用相同的 overload、method-set 與 access 證據。`var` 只接受非泛型的直接 object creation，並在 v2 receipt 保存 declaration、initializer、scope 與 project-resolved type；factory call、一般賦值、宣告前、scope 外、欄位、泛型及任意 expression receiver 都保持不解析。`super` 仍要求唯一 exact 直接父類，並保留 caller-to-super path、繞過子類 override。
