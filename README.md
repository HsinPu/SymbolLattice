...
> [!IMPORTANT]
> v0.333.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 否定 pattern guard 支援 `if (!(value instanceof DirectType name))`。若 then 分支以可證明的 top-level `return` 或 `throw` 結束，v14 會把 binding 延伸至同 lexical block 的後續作用域；有 `else` 時，v15 會在 else body 內建立 definitely-matched binding，並只在 then 分支可證明 abrupt completion 時繼續延伸到 if 之後。v16／v17 支援無標籤 loop `break`／`continue`，v18／v19 支援依名稱精確解析的 labeled exit，v20／v21 再支援傳統 `case:` 群組內的無標籤 `switch break`。switch binding 只延伸到同一 case group 或其明確 lexical block，絕不跨越下一個 label 或 switch 外；receipt 保存 switch body、case group 與 label ranges。巢狀 loop 仍優先接管 `break`。條件式 exit、不安全巢狀 guard、arrow rule、missing／duplicate label、`continue` 到非迴圈 label、同名既有 binding、泛型、錯誤極性與作用域外都保持不解析。
