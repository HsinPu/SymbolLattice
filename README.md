...
> [!IMPORTANT]
> v0.320.0 是開發預覽版。MCP 查詢工具唯讀；但 `serve --mcp` 預設會啟動獨立的本機 auto-sync watcher，可能更新專案的 `.symbol-lattice` 索引。加入 `--no-auto-sync` 可停用它。
...
- Java 未初始化的明確型別區域變數，只有在同一 lexical block 內出現可證明的直接 `receiver = new DirectType(...)` 賦值後，才能作為 member-call receiver。解析器要求宣告型別與實際建立型別完全相同，或由 exact hierarchy 證明 reference widening；賦值前、巢狀條件區塊、型別不相容及其他一般控制流都 fail closed。v6 receipt 保存宣告、scope、賦值與 initializer range、canonical types、相容性決策及完整 hierarchy path。
