...
v0.520.0 改善小寫 `.js`／`.mjs`／`.cjs` 來源明確副檔名的跨檔 import：指定來源檔案存在時，不再因同名其他 JavaScript 格式而保留歧義。固定三專案的規則候選命中由 8,859 增至 8,870，增加 11 筆；這個集合包含宣告與結構關係，不代表完整語言覆蓋率。評分器同步揭露未測量的 precision 與負向案例的實際模板數，TypeScript 與未指定副檔名的歧義防護保留。
