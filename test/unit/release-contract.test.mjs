...
async function fixture() {
  const root = await mkdtemp(join(tmpdir(), "SymbolLattice-release-contract-"));
  ...
  const tarballPath = join(outputDirectory, "SymbolLattice-0.255.0.tgz");
...
describe("GitHub release artifact contract", () => {
  it("derives the public GitHub release tarball name independently of npm's lowercase filename", () => {
    expect(expectedReleaseTarballName("@hsinpu/symbollattice", "0.255.0")).toBe(
      "SymbolLattice-0.255.0.tgz"
    );
  });
  ...
  it("writes a checksum and deterministic manifest tied to the source commit", async () => {
    ...
    expect(result).toMatchObject({
      ...
      artifact: {
        filename: "SymbolLattice-0.255.0.tgz",
        sizeBytes: 22,
        checksumFilename: "SymbolLattice-0.255.0.tgz.sha256"
      },
      installation: {
    ...
    expect(checksum).toBe(`${result.artifact.sha256}  SymbolLattice-0.255.0.tgz\n`);
  ...
  it("refuses a tarball whose basename does not match npm's package/version name", async () => {
    ...
    const wrongPath = join(outputDirectory, "SymbolLattice-latest.tgz");
