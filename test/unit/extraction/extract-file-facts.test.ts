...
describe("source extraction", () => {
  ...
  it("retains final Flask Blueprint exports from a package initializer", () => {
    const facts = extractFileFacts({
      filePath: "app/routes/__init__.py",
      language: "python",
  ...
  it("rejects rebounded and non-initializer Flask Blueprint exports", () => {
    const reboundFacts = extractFileFacts({
      filePath: "app/routes/__init__.py",
      language: "python",
      sourceText: [
        "from .catalog import blueprint as public_blueprint",
  ...
  it("retains direct GoFrame var controller aliases only with matching pointer types", () => {
    const facts = extractFileFacts({
      filePath: "api/goframe-var-controller-alias.go",
      language: "go",
      sourceText: [
        "package api",
        "",
        ...
        "type Controller struct{}",
        "type SecondController struct{}",
        "type ErasedController struct{}",
        "type GroupedController struct{}",
        "type FirstMultiController struct{}",
        "type SecondMultiController struct{}",
        "",
        "func Register() {",
        "  var controller = &Controller{}",
        "  var second *SecondController = new(SecondController)",
        "  var erased interface{} = &ErasedController{}",
        "  var (",
        "    grouped = &GroupedController{}",
        "  )",
        "  var first, secondMulti = &FirstMultiController{}, &SecondMultiController{}",
        '  g.Server().Domain("api.example.test").Group("/v1").Bind(controller, second, erased)',
        '  g.Server().Group("/grouped").Bind(grouped)',
        '  g.Server().Group("/multiple").Bind(first, secondMulti)',
        '  g.Server().Group("/callback", func(controller *ghttp.RouterGroup) {',
        "    controller.Bind(controller)",
        "  })",
        "  controller = &Controller{}",
    ...
    expect(facts.goFrameStandardRouterFacts?.controllerBindings).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({ controllerName: "ErasedController" }),
        expect.objectContaining({ prefix: "/callback" }),
        expect.objectContaining({ prefix: "/grouped" }),
        expect.objectContaining({ prefix: "/multiple" }),
  ...
  it("retains direct Java Spring Boot @ConfigurationProperties literal-prefix facts", () => {
    const facts = extractFileFacts({
      filePath: "src/config/CacheProperties.java",
      language: "java",
      sourceText: [
        "import org.springframework.boot.context.properties.ConfigurationProperties;",
        "",
  ...
  it("rejects unproven, dynamic, value-alias, and multi-attribute Java @ConfigurationProperties forms", () => {
    const directImportFacts = extractFileFacts({
      filePath: "src/config/UnsupportedConfigurationProperties.java",
      language: "java",
      sourceText: [
        "import org.springframework.boot.context.properties.ConfigurationProperties;",
        '@ConfigurationProperties(value = "value.alias") class ValueAlias {}',
        '@ConfigurationProperties(prefix = "multiple.attributes", ignoreUnknownFields = false) class Multiple {}',
        "@ConfigurationProperties(prefix = PREFIX) class Dynamic {}"
      ].join("\n")
    });
    const wildcardImportFacts = extractFileFacts({
      filePath: "src/config/WildcardConfigurationProperties.java",
      language: "java",
      sourceText: [
        "import org.springframework.boot.context.properties.*;",
        '@ConfigurationProperties(prefix = "unproven.prefix")',
