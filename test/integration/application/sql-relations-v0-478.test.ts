...
describe("PostgreSQL project relations v0.478", () => {
  it("resolves unique cross-file foreign-key and table inheritance targets", async () => {
    ...
    expect(graphStore.getActiveGraphBundle(projectPath).extractorVersion).toContain("multi-language-ast-v430");
    expect(graphStore.getActiveGraphBundle(projectPath).resolverVersion).toContain("project-resolver-v206");
