...
describe("Markdown reference-link project relations v0.482", () => {
  it("resolves unique reference links to indexed local files", async () => {
    ...
    expect(store.getActiveGraphBundle(projectPath).extractorVersion).toContain("multi-language-ast-v387");
    expect(store.getActiveGraphBundle(projectPath).resolverVersion).toContain("project-resolver-v192");
