...
describe("Shell structural v1.2.2 persistence", () => {
  ...
  it("invalidates stale v327 facts and persists v330 direct-root structure", async () => {
    ...
    expect(ARTIFACT_FACTS_EXTRACTOR_VERSION).toBe("multi-language-ast-v362");
    expect(PROJECT_RESOLVER_VERSION).toBe("project-resolver-v167");
    ...
    expect(facts).toMatchObject({
      filePath: "scripts/deploy.sh",
      extractorVersion: "multi-language-ast-v362"
    });
    expect(facts?.symbols.filter(({ kind }) => kind === "function").map(({ name }) => name))
      .toEqual(["left", "right"]);
