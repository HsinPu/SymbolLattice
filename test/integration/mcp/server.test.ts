...
describe("SymbolLattice MCP server", () => {
  ...
  it("exposes read-only retrieval tools and forwards projects and filters", async () => {
    ...
    expect(exploreTool?.description).toContain("concise Markdown");
    expect(exploreTool?.outputSchema).toBeUndefined();
    ...
    expect(result.content[0]?.text).toContain("**Exploration: missing**");
    expect(result.content[0]?.text).toContain("No exact symbol found for `missing`.");
  ...
  it("shares exact source coverage from explore into context through one MCP session", async () => {
    ...
    expect(first.content[0]?.text).toContain("**Source Code**");
    expect(first.content[0]?.text).toContain(sourceText);
