...
> [!IMPORTANT]
> v0.331.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- A negated Java pattern guard supports `if (!(value instanceof DirectType name))`. When the then branch ends in a proven top-level `return` or `throw`, v14 extends the binding through the remaining scope in the same lexical block. With an `else`, v15 creates a definitely-matched binding throughout the else body and extends it beyond the if only when the then branch has proven abrupt completion. v16 adds unlabeled `break` and `continue` guards; v17 carries the same exact loop-target proof into a negated `else`, retaining separate active regions for the else body and following scope. Both accept a loop exit only when the guard is inside the physical block of an exactly proven while, do, for, or enhanced-for target, and retain the loop target kind and range. Receipts retain the guard, condition, pattern, up to four parenthesis layers, body, abrupt statement, target, active region, and exact scope. Fall-through, conditional exits, labeled jumps, unbraced loops, switch breaks, existing same-name bindings, generics, wrong polarity, and uses outside that block remain unresolved.
