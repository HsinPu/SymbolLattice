...
v0.510.0 promotes Pascal to bounded project relation evidence. Fixed `fpc/FPCSource` `release_3_2_2` commit `0d122c49534b480be9284c21bd60b53d99904346` and a compiler-parse gate plus independent source oracle verify explicit program-level `uses`, one tracked unit, and one exported zero-argument bare routine call: 300 TP, 0 FP, 0 FN, 0 evidence-invalid, with 150/150 negatives passing. The large corpus extracts all 2,787 currently discoverable main files; its independent oracle admits one compiler-confirmed candidate and the product resolves 1/1 exactly, while 13,149 `.pp`/`.inc` candidates remain outside the existing discovery boundary. Overloaded or ambiguous exports, qualified/member or virtual dispatch, include/macro/conditional forms, forward/external declarations, package resolution, and runtime semantics remain nonclaims. The parse gate uses the official 3.2.3 fixes snapshot; the stable 3.2.2 installer was not executed and that stability limitation remains explicit in the evidence.
