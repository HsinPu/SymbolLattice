...
> [!IMPORTANT]
> v0.115.0 is an early developer release. Run it from source; the npm package is not published.
...
- Covers frontend, backend, JVM, systems, data, IaC, template, and schema languages. Rust route support includes conservative Axum, Actix Web App/resource/scope-builder, and Rocket scanners.
...
- Adds direct Rust Actix Web `web::scope("/prefix")` builder routes: static prefix composition for scoped `.route(...)`, mounted resources, and nested scopes.
- Retains framework-specific evidence for every scoped route. Unmounted, dynamic, shadowed, wrapped, and trailing-slash scopes emit no route.
...
- Actix Web currently accepts directly imported HTTP attribute macros or contiguous `App::new()` `.route(...)` / `.service(web::resource(...) | web::scope(...))` static chains. A scope requires a literal slash-prefixed, non-trailing-slash prefix (except root `/`) and may contain only direct routes, resources, or nested scopes; `configure`, attribute-service mounts, guards, wrappers, dynamic paths, and runtime composition are deferred. Rocket currently supports direct imported HTTP attribute macros only.
