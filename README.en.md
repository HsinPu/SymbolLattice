...
> [!IMPORTANT]
> v0.183.0 is a developer preview and is not published to npm. Run it from source.
...
- Spring Web now recognizes direct Java and Kotlin handlers with `@RequestMapping(method = …)`, supporting GET, POST, PUT, PATCH, DELETE, HEAD, OPTIONS, and TRACE.
...
- Every route links to its local handler with `exact` evidence. Multi-method and conditional mappings, dynamic paths, alias/wildcard imports, default ALL mappings, and runtime routing remain deliberately excluded.
...
- Spring Web supports direct top-level concrete Java/Kotlin controllers, literal HTTP shortcut annotations, and one provable `RequestMethod` enum on method-level `@RequestMapping`. Default ALL, multi-method, conditions, proxies, nested declarations, and runtime routing remain out of scope.
