...
> [!IMPORTANT]
> v0.310.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- Explicit Java `this.method()` and `super.method()` calls, parameter/block-local receivers with direct reference types, and `var receiver = new DirectType(...)` reuse the same overload, method-set, and access evidence. `var` accepts only a direct non-generic object creation; its v2 receipt preserves declaration, initializer, scope, and project-resolved type. Factory calls, assignments, pre-declaration or out-of-scope names, fields, generics, and arbitrary expressions remain unresolved. `super` still requires one proven exact direct superclass, retains the caller-to-super path, and bypasses child overrides.
