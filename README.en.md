...
> [!IMPORTANT]
> v0.334.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- A negated Java pattern guard supports `if (!(value instanceof DirectType name))`. v14/v15 cover proven `return`/`throw` exits and negated `else`; v16-v19 cover unlabeled and named loop exits; v20/v21 cover `switch break` inside traditional `case:` groups. v22/v23 add a top-level `yield` in a switch-expression arrow-rule block when the expression is proven to occur in a `return`, variable initializer, assignment, or outer `yield`; receipts retain the switch body, rule, rule body, label, expression context, and exact scope. Traditional-case bindings never cross the next label, while arrow-rule bindings never escape their current rule block. A nested loop still owns the nearest `break`. Colon-style `yield`, statement switches, conditional exits, unsafe nested guards, existing same-name bindings, generics, wrong polarity, and out-of-scope uses fail closed.
