...
> [!IMPORTANT]
> v0.311.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- Explicit Java `this.method()` and `super.method()` calls, parameter/block-local/direct declaring-class field receivers with direct reference types, and `var receiver = new DirectType(...)` reuse the same overload, method-set, and access evidence. Fields support bare names and `this.field`, retaining type, declaration/class scope, visibility, and static proof; lexical shadowing, static-context violations, duplicate names, and unsupported types fail closed. `var` accepts only direct non-generic object creation. Inherited fields, factory calls, assignments, out-of-scope names, generics, and arbitrary expressions remain unresolved. `super` still requires one proven exact direct superclass and retains its complete path.
