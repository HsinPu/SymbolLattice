...
> [!IMPORTANT]
> v0.328.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- A negated Java pattern guard can bind after `if (!(value instanceof DirectType name))` when a proven `return` or `throw` prevents the guarded path from completing normally. The v14 receipt records the guard, condition, pattern, up to four parenthesis layers, exit body, abrupt statement, and remaining scope in the same lexical block; a block body must end in a top-level abrupt statement. Fall-through, conditional exits, `else`, existing same-name bindings, generics, wrong polarity, and uses outside that block remain unresolved.
