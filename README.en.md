...
- `explore` supports both exact symbols and bounded question mode. Safe explicit project-relative paths rank first; other focus candidates combine symbol, filename, and exact graph-connection evidence with persisted generated-source worth (`0.3`). This is a soft numeric ranking rather than a generated-file filter, and explicitly named generated files are exempt. Receipts separate raw score from ranking score and include classifier evidence. It selects at most four files, eight focuses, two focuses per file, and sixteen connections. Question mode checks at most sixteen focus pairs for exact paths within four hops, retaining up to four spines and eight unselected bridge symbols; heuristic and unresolved relations never fill a path.
