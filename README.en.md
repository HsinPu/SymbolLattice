...
> [!IMPORTANT]
> v0.300.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- Java can follow a direct static-factory chain such as `Factory.create().execute()` and resolve both exact `calls` through the proven top-level return type. Fixed and varargs candidates are filtered by actual argument count first; same-arity candidates are selected only when primitive/string literals or non-generic `new Type(...)` expressions prove one exact identity-compatible declaration and rule out every other candidate. `callArity`/`callType` receipts list every candidate, range, ordered type, and compatibility. Unknown variables, `null`, boxing, widening, subtyping, casts, generic/array types, instance factories, shadowed receivers, and inherited methods are never guessed into type-exact edges.
