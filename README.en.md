...
> [!IMPORTANT]
> v0.301.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- Java can follow a direct static-factory chain such as `Factory.create().execute()` and resolve both exact `calls` through the proven top-level return type. Fixed and varargs candidates are filtered by actual argument count, then selected from primitive/string literals, explicit primitive casts, or non-generic `new Type(...)` evidence. Primitive identity and Java widening (`byte → short → int → long → float → double`, and `char → int → long → float → double`) use fixed-arity phase precedence and a per-argument Pareto-best rule; no unique best candidate means no edge. `callArity`/`callType` receipts expose invocation mode, every conversion, distance, and the selection. Unknown variables, `null`, boxing, unboxing, reference subtyping/casts, generic/array types, instance factories, shadowed receivers, and inherited methods are never guessed into exact edges.
