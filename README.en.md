...
> [!IMPORTANT]
> v0.295.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- TypeScript function and class-method input/return types become exact `accepts`/`returns` relations only when a local, type-only import, or re-export proof exists. Type parameters, built-in wrappers, qualified names, and unimported same-name types are never guessed into exact edges.
