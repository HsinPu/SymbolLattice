...
> [!IMPORTANT]
> v0.312.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- Explicit Java `this.method()` and `super.method()` calls, parameter/block-local/field receivers with direct reference types, and `var receiver = new DirectType(...)` reuse the same overload, method-set, and access evidence. Bare names and `this.field` can follow one exact class `extends` chain to the nearest declaration while retaining the owner path, visibility, package access, and static proof. Lexical shadowing, an inaccessible or unknown nearest declaration, cycles, competing owners, bounds, and static-context violations fail closed. `var` accepts only direct non-generic object creation. Interface fields, `super.field`, nestmate private access, assignments, generic or array fields, and classpath guesses remain unresolved. `super.method()` still requires one proven exact direct superclass and retains its complete path.
