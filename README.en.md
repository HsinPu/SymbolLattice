...
> [!IMPORTANT]
> v0.333.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- A negated Java pattern guard supports `if (!(value instanceof DirectType name))`. v14/v15 cover proven `return` or `throw` exits and negated `else`; v16/v17 cover unlabeled loop `break`/`continue`; v18/v19 add exact name-resolved labeled exits. v20/v21 now cover unlabeled `switch break` inside traditional `case:` groups. A switch binding ends at the same case group or its explicit lexical block and never crosses the next label or escapes the switch; receipts retain the switch body, case group, and label ranges. A nested loop still owns the nearest `break`. Conditional exits, unsafe nested guards, arrow rules, missing or duplicate labels, `continue` to a non-loop label, existing same-name bindings, generics, wrong polarity, and out-of-scope uses remain unresolved.
