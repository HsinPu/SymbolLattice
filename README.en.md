...
> [!IMPORTANT]
> v0.321.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- An uninitialized, explicitly typed Java local becomes a member-call receiver after a proven direct `receiver = new DirectType(...)` assignment in the same lexical block. It can also join one exhaustive `if/else` made of exactly two block branches that each contain one direct object-creation assignment. The declared type and every created type must be identical or connected by an exact reference-widening hierarchy path. Missing `else`, `else if`, extra branch statements, incompatible types, and later reassignment fail closed. v6/v7 receipts retain declaration, scope, statement, condition, per-branch assignment and initializer ranges, canonical types, compatibility decisions, and complete hierarchy paths.
