...
- `explore` supports both exact symbols and bounded question mode. Safe explicit project-relative paths rank first; other focus candidates combine symbol and filename evidence, selected-candidate connections, and bounded exact one-hop graph mass with persisted generated-source worth (`0.3`). Graph mass deduplicates each relation-neighbor pair, admits at most 32 relationships and 120 points, weights stronger behavioral relations above imports/references, and ignores containment, heuristic, and unresolved edges. This is a soft numeric ranking rather than a generated-file filter, and explicitly named generated files are exempt. Receipts separate raw and ranking scores and expose classifier, graph-mass counts, relation weights, omissions, and caps. It selects at most four files, eight focuses, two focuses per file, and sixteen connections. Question mode checks at most sixteen focus pairs for exact paths within four hops, retaining up to four spines and eight unselected bridge symbols; heuristic and unresolved relations never fill a path.
