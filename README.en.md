...
> [!IMPORTANT]
> v0.320.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- An uninitialized, explicitly typed Java local becomes a member-call receiver only after a proven direct `receiver = new DirectType(...)` assignment in the same lexical block. The declared and created types must be identical or connected by an exact reference-widening hierarchy path; calls before assignment, nested conditional assignments, incompatible types, and other general control flow fail closed. v6 receipts retain declaration, scope, assignment and initializer ranges, canonical types, the compatibility decision, and the complete hierarchy path.
