...
> [!IMPORTANT]
> v0.293.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- `explore` supports exact symbols and bounded question mode. Safe explicit paths rank first; other candidates combine lexical evidence, connections, exact one-hop graph mass, and bounded graph diffusion across at most four hops before persisted generated worth (`0.3`) and test, icon, or localization worth (`0.5` each) are applied. Diffusion uses lexical-tier seeds, equal restart weight per seed file, a `0.25` restart probability, and weighted exact relationships; receipts expose convergence, residual, truncation, and per-candidate contribution. A general question excludes unrequested low-value roles only when the current classifier proves at least two distinct production candidate files; insufficient evidence keeps the soft penalty. Role intent and explicit files are exempt independently. Selection remains bounded to four files, eight focuses, two focuses per file, sixteen connections, and exact paths within four hops. This version reranks existing lexical or explicit-file candidates; it does not add graph-connected files with no textual match.
