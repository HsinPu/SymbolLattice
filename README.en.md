...
> [!IMPORTANT]
> v0.277.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- `node`, `investigate`, and `file` attach one tool-independent identity to each exact persisted source delivery. Within one MCP session, only the same file, full-file UTF-16 offsets, canonicalized content hash, project, and generation become an explicit back-reference; `sourceSessionMode: "full"` re-emits source. State is bounded and session-local, generation changes reset it, and malformed identity evidence fails open by returning the original response.
