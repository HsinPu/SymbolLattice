...
> [!IMPORTANT]
> v0.315.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- Explicit Java `this.method()` and `super.method()` calls, parameter/block-local/field receivers with direct reference types, and `var receiver = new DirectType(...)` reuse the same overload, method-set, and access evidence. Fields support bare names, `this.field`, `super.field`, and `InterfaceName.FIELD.method()` when an explicit import, same-package type, or fully qualified spelling proves the interface owner. Ordinary lookup prefers the nearest class declaration, then follows the exact `implements`/interface `extends` graph for interface constants with subinterface hiding, class precedence, and ambiguity fail-closed. Interface constants retain their implicit `public static final` proof, qualified owner, complete owner path, access, and bounds. A same-name local or field anywhere in the caller hierarchy, a class-qualified field, inaccessible or unknown types, cycles, competing owners, bounds, and static-context violations fail closed. `var` accepts only direct non-generic object creation. Nestmate private access, assignments, generic or array fields, and classpath guesses remain unresolved. `super.method()` still requires one proven exact direct superclass and retains its complete path.
