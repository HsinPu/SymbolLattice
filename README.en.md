...
- `explore` primary excerpts, exact call sites, and bridge windows share one 24,000-character ceiling. Windows are allocated by focus score and spine evidence; when capacity permits, multiple windows keep a 256-character floor, and any one window receives at most 70%. Every excerpt comes from the same active generation, and ranges overlapping primary excerpts are not emitted again.
