...
> [!IMPORTANT]
> v0.332.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- A negated Java pattern guard supports `if (!(value instanceof DirectType name))`. When the then branch ends in a proven top-level `return` or `throw`, v14 extends the binding through the remaining scope in the same lexical block. With an `else`, v15 creates a definitely-matched binding throughout the else body and extends it beyond the if only when the then branch has proven abrupt completion. v16/v17 support unlabeled `break` and `continue` guards and negated `else`; v18/v19 add name-resolved labeled exits. `continue label` accepts only a directly labeled while, do, for, or enhanced-for statement, while `break label` may target one unique enclosing labeled statement. Labeled receipts also retain the label name/range, full target range, and actual target-body range. Every policy retains the guard, condition, pattern, up to four parenthesis layers, body, abrupt statement, active region, and exact scope. Fall-through, conditional exits, missing or duplicate labels, `continue` to a non-loop label, unbraced loops, switch breaks, existing same-name bindings, generics, wrong polarity, and uses outside that block remain unresolved.
