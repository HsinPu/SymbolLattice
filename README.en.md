...
> [!IMPORTANT]
> v0.303.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- Java can follow a direct `Factory.create().execute()` static-factory chain and resolve both exact `calls` through a proven top-level return type. Fixed/varargs candidates are filtered by argument count, then by primitive/string literals, explicit primitive casts, or non-generic `new Type(...)` evidence. Primitive identity/widening and project-local exact `extends`/`implements` reference widening participate in overload selection; reference candidates use parameter specificity rather than argument distance alone. When the returned type has no same-name method, exact hierarchy evidence can also select one uniquely most-specific inherited class or interface owner, including a valid default-interface method. Every hierarchy traversal is bounded to 16 hops and 256 types; a bound or owner ambiguity produces no edge. `callType` and `callDispatch` receipts expose selection reasons, candidate owners, bounds, and every hierarchy edge. Unknown variables, `null`, boxing/unboxing, reference casts, generic/array types, instance factories, shadowed receivers, cross-owner overload merging, and precedence between unrelated class/interface owners remain unresolved.
