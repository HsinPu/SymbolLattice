...
> [!IMPORTANT]
> v0.299.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- Java can follow a direct static-factory chain such as `Factory.create().execute()` and resolve both exact `calls` through the proven top-level return type. Fixed and varargs overloads are selected only when exactly one declaration accepts the actual argument count; each edge receipt lists every candidate, its arity range, and applicability. Same-arity or overlapping-varargs ambiguity, instance factories, shadowed receivers, wildcard or unimported types, parameter-type conversions, and inherited methods remain unresolved.
