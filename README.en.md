...
> [!IMPORTANT]
> v0.298.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- Java can follow a direct static-factory chain such as `Factory.create().execute()` and resolve both exact `calls` through the proven top-level return type. This currently requires an explicit import, fully qualified receiver, or one unique same-package receiver, plus one static factory and one directly declared target method; overloads, instance factories, shadowed receivers, wildcard or unimported types, and inherited methods remain unresolved.
