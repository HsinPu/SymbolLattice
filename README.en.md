...
> [!IMPORTANT]
> v0.309.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- Explicit Java `this.method()` and `super.method()` calls, plus method-parameter and block-local receivers with direct reference types, reuse the same overload, method-set, and access evidence. Parameter/local bindings are tied to lexical scope, declaration order, type, source ranges, and the active generation. Undeclared, pre-declaration, out-of-scope, field, generic, and arbitrary-expression receivers are never guessed into exact edges. `super` still requires one proven exact direct superclass, retains the caller-to-super path, and bypasses child overrides.
