...
v0.511.0 preserves the v0.510 Pascal bounded unit relation and adds content-gated `.pp` discovery. `.pp` is not added to the generic extension map; only files that, after comment/string sanitization, contain exactly one `unit`, `program`, or `library` header with no conditional, macro, or include taint are classified as Pascal. The 300-positive and 150-negative discovery fixtures, including Puppet, `.inc`, fake-header, multiple-header, tainted, and malformed negatives, all pass, while existing `.pas`/`.lpr`/`.dpr`/`.dpk` behavior remains unchanged. Pascal overload/ambiguous exports, qualified/member or virtual dispatch, forward/external declarations, package resolution, and runtime semantics remain nonclaims; `.pp` admission does not imply Puppet support.
