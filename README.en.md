...
> [!IMPORTANT]
> v0.306.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- Java can follow a direct `Factory.create().execute()` static-factory chain through a proven top-level return type. Factories may be public static or package-private/protected static in the caller's package; target methods additionally allow cross-package protected access when both the caller-subclass relation and receiver constraint are proven. `callType`, `callDispatch`, and access receipts expose signatures, owners, packages, bounds, and every hierarchy edge. Private access, cross-package package-private access, unproven inherited protected static calls, unknown types, boxing, and generics remain unresolved.
