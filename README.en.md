...
> [!IMPORTANT]
> v0.304.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- Java can follow a direct `Factory.create().execute()` static-factory chain and resolve both exact `calls` through a proven top-level return type. Its method set merges different-signature overloads across direct and inherited owners; equal signatures apply direct-owner, class-over-interface, then unique-most-specific precedence, while interface static methods are never treated as instance members. Cross-file chains admit only public factory/target methods, including implicit-public interface methods. Fixed/varargs phases, primitive/reference widening, and parameter specificity select the overload. Hierarchy traversal is fixed at 16 hops and 256 types; a bound or relevant-signature owner ambiguity produces no edge. `callType` and `callDispatch` receipts expose canonical signatures, owner kinds, selection reasons, bounds, and every hierarchy edge. Unknown variables, `null`, boxing/unboxing, reference casts, generic/array types, instance factories, shadowed receivers, protected/package/private caller-context access, and full override-equivalent erasure remain unresolved.
