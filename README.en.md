...
> [!IMPORTANT]
> v0.305.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- Java can follow a direct `Factory.create().execute()` static-factory chain through a proven top-level return type. Its method set merges direct and inherited overloads and applies Java owner precedence. Target methods may be public, package-private/protected in the caller's package, or cross-package protected when both the caller-subclass relation and receiver constraint are proven. `callType`, `callDispatch`, and access receipts expose signatures, owners, packages, bounds, and every hierarchy edge. Factories remain public static only; private, cross-package package-private, unproven protected access, unknown types, boxing, and generics remain unresolved.
