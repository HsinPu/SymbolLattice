...
> [!IMPORTANT]
> v0.289.0 is a developer preview. MCP query tools are read-only, but `serve --mcp` starts a separate local auto-sync watcher by default. That watcher can update the project's `.symbol-lattice` index; add `--no-auto-sync` to disable it.
...
- `explore` supports exact symbols and bounded question mode. Safe explicit paths rank first; other candidates combine lexical evidence, selected-candidate connections, and bounded exact one-hop graph mass with persisted generated-source worth (`0.3`) and test-source worth (`0.5`). General queries only lower test-source ranking; they never hide candidates. An explicit test file or standalone `test`, `spec`, or `verify` intent exempts the test penalty. Receipts separate raw and ranked scores and expose both classifiers, intent, weights, graph-mass counts, and caps. Selection remains bounded to four files, eight focuses, two focuses per file, sixteen connections, and exact paths within four hops.
